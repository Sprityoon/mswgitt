...
| **[docs/pitfalls.md](docs/pitfalls.md)** | 🔴 **이 프로젝트에서 실제로 사고를 낸 함정 17건.** 대부분 에러 없이 조용히 틀린다 |
...
**편집 수단이 강제된다**: `.model` → `ModelBuilder` · `.ui` → `UIBuilder` · `.map` → `MapBuilder`. 직접 편집은 훅이 차단한다. 자세한 레인 표는 [AGENTS.md §2](AGENTS.md).
