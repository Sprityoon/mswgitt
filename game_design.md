...
- [ ] **20-I 자원·가구 Trigger 부여** (T100 — ⚖️ 2026-08-01 보스 T96 (C) 확정, **T75의 선행**): 자원 6종(`Tree1`·`Tree2`·`Stone`·`IronNodeResource`·`GrownGrass`·`Crop_Carrot`) + 가구 6종(`Bed`·`CookingPot`·`Furnace`·`WoodenChest`·`AnimalPen`·`MonsterWard`)에 `TriggerComponent` 부여로 20-G′ 접지선 자동 산출에 편입. 🔴 **최우선 제약 = T81(차단)·T82(상호작용 범위)·T83(페이드) 동작 불변** — Trigger가 이 프로젝트에서 4역할을 겸하므로 정렬만 얻고 나머지 셋은 그대로여야 한다. 착수 1단계는 **T96 선행조사 3항목**(ⓐ `Big Stone1/2`만 Trigger인 이유 ⓑ 멤버십 없는 Trigger가 통행을 안 막는지 코드 확정 ⓒ 가구 Trigger 시 `AimFootprint`→Trigger AABB 자동 전환의 범위 변화 수치 대조) — **ⓒ의 결론이 T75 소품 스펙을 확정**한다.
