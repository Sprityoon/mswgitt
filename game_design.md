...
- [ ] **21-A 물 타일 기반** (T90 — 🔴 **부분 이행 → 반려, 배치분은 T98로 재작업**): `wall.tileset` 미사용 슬롯에 `Water` 타일 + `IsWaterTileName` + 물 셀 `Movable=false`(통행 불가) + 자원 스폰 억제 + 미니맵 물색 + `build_maps.cjs` 고정 수역 배치·산출 검사. **마스크 함수 diff 0이 완료 조건.**
  - ✅ **인정분**: `wall.tileset`의 `Water` 타일 정의 + 판정 함수. ❌ **미이행**: 고정 수역 **실배치**(Change ⑥) — 제작자 Play "물이 배치가 안 돼 있어" + 지휘자 실측(**2026-08-01 재확인: 4개 맵 `"Water"` 참조 0건**).
  - ⚖️ **2026-07-28 보스 결정 — 물 페인팅은 제작자가 Maker에서 직접, 에이전트는 프린지 보정만**(→ **T98**). 근거 = ⓐ `ResourceSpawner` 914~916행 "맵 타일은 Maker 손편집이 소스 오브 트루스" ⓑ `MapBuilder` 타일 페인팅 미지원(builder-protocol §1.6 갭) ⓒ `build_maps.cjs --force`가 town 손배치를 전량 파괴. **🔑 핵심 통찰**: L2 관점에서 물 셀 = 그냥 L2 홀이므로 **전용 물가 타일 불요**, 기존 `digHole` 마스크 연산이 둘레를 그대로 처리한다.
  - 🔴 **Phase 21 완성 게이트** — 물이 놓이기 전까지 21-B의 사냥터 경로는 검증 불가.
- [ ] **21-B 낚시 = 물 타일 인접 판정** (T91 — 코드 완료 · refresh Error=0 | ⏳ **물 미배치로 사냥터 경로 미검증**, 영지는 21-C 파기로 자체 검증 가능): 조준 셀이 물이면 F로 캐스팅. **마을은 픽스처 유지 / 영지·사냥터 픽스처 제거** — 판정 우선순위 = 픽스처 `SpotType` → 없으면 맵 기반. 맵 종류는 T37 mapKind 규약 재사용, 어종은 CSV 행 추가. ⚠️ **T85(영지 낚시터 복구, 커밋 `ce13617`)를 되돌리는 티켓** — 같은 날 설계 변경에 따른 것으로 회귀가 아니다.
- [ ] **21-C 영지 물 파기** (T92 — 코드 완료 · refresh Error=0 | 런타임 검증 보류(제작자)): 지형 편집에 `dig_water`/`fill_water` action 추가(영지 전용 게이트). 되메우기 필수, **자기 자신이 갇히는 배치 방지 가드** 필수(물이 통행 불가이므로).
...
- [ ] **20-I 가구 Trigger 부여 + Trigger 전수 재감사** (T100 — ⚖️ 2026-08-01 보스 T96 (C) 확정, **T75의 선행**, 🔄 2026-08-01 실측으로 범위 재작성): **자원 6종은 이미 전부 Trigger 보유 → 대상 아님**(T96 표의 "미보유"는 규칙 13 갭에 의한 오독). 실제 대상은 **가구 6종**(`Bed`·`CookingPot`·`Furnace`·`WoodenChest`·`AnimalPen`·`MonsterWard`)이며, 이들은 Trigger·PhysicsCollider가 **둘 다 없다**. 🔴 그래서 `ObstacleQuery`의 `OverlapAll(CollisionGroups.TriggerBox, ...)` 후보 수집에 걸리지 않아 **가구 통행 차단이 지금 작동하지 않을 가능성**이 있고, 그 판정이 이 티켓의 핵심 가치다. 착수 1단계 = **갭 우회 읽기 경로로 Trigger 전수 재감사**(T96의 "22/23" 수치 교체). 🔴 **최우선 제약 = T81(차단)·T82(상호작용 범위)·T83(페이드) 동작 불변** — Trigger는 이 프로젝트에서 4역할을 겸하므로 정렬만 얻고 나머지 셋은 그대로여야 한다. **T82 범위 전환 대조 결과가 T75 소품 스펙을 확정**한다.
