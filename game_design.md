...
- [x] **15-C 낚시** (T18 — **Play 검증 PASS 2026-07-13**): 낚싯대(`ToolType=rod`) + 낚시터 픽스처(영지 연못/마을/사냥터 물가 — 타일 지형 불가침) + `FishDataSet`(SpotType별 어종) + 원버튼 타이밍 미니게임(캐스팅→랜덤 대기→`!`→0.8초 윈도우). **→ v2 릴링 개편 코드 완료(T64, 2026-07-18 지휘자 직접 — Play 검증 보류)**: 입질 후 랜덤 미스 폐지(입질=자동 릴링 진입, 실패=텐션 초과 줄 끊김뿐), F/BtnInteract 홀드-릴리즈 릴링(위험 ⚠ 중 릴리즈) + HUD `FishingGauge`(진행 금색/텐션 적색) + 낚시 숙련 레벨(`FishingLevel`/`FishingXp` 영속 — 어종 풀 개방·텐션 완화, `FishDataSet.Difficulty/MinFishingLevel/FishingXp` 컬럼 + 신규 `FishingDifficultyDataSet` 티어 테이블).
...
- [ ] **19-D 상점거리 소품 및 생활감** (T75 — **대기**): 벤치, 가로등, 울타리 등 데코 14종 다중 배치. **착수 게이트 = 배치 N(T84~T87) 완료 후**(town.map 파일 공유).
- [ ] **19-E 마을 랜드마크 건물** (T76 — **대기**): 스카이라인 형성용 대형 건물(여관, 헛간, 시계탑) 배치. 원본 아트는 `scratch/artwork_rework/source/{inn,barn,clocktower}.png`에 이미 확보돼 있으나 `.sprite`·`.model`화는 미수행. **착수 게이트 = T75 이후**(town.map 순차).
...
- [ ] **19-G 필드 및 바이옴 변주** (T78 — **대기**): 다양한 나무·바위로 사냥터 다변화. ~~낚시터 모델 리스킨~~ → **무티켓 커밋 `38ae03c`에서 이미 완료**(`FishingSpot_Pond` = `ecb83722…`, spec F9) — 범위에서 제외(🧭 2026-07-25).
