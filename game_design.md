...
- **연동 검증**: `msw-maker-mcp`를 통한 `refresh` 및 빌드 로그 검증 의무화. **Play 런타임 검증은 제작자 전담.**
...
| [docs/tile-scheme.md](./docs/tile-scheme.md) | 타일 지형 문법 단일 소스 |
...
- **단계적 사냥터 (Tiered / Chained — 디아블로2식)**: 사냥터는 **여러 구역(area)이 포탈로 연결된 단계 구조**다.
  - **체인 진행 (⚖️ 2026-09-22 확정)**:
    - **1막 (초보 흙 벌판)**: `hunt01` (1구역) ➔ `hunt02` (2구역) ➔ `hunt03` (3구역)으로 도보 전진 포탈(`PortalForward`)이 이어지며, 3구역 끝자락 포탈을 통해 **1막 관문 보스 「슬라임킹」 (`template_boss`)**에 도전한다.
    - **상위 바이옴 순차 해금**: 관문 보스를 격파할 때마다 상위 컨셉 맵이 해금된다 (`SlimeKing` ➔ `rocky01` 바위 고원, `StoneGolem` ➔ `desert01` 모래 언덕, `Deu` ➔ `snow01` 만년 설원).
    - **템플릿과 인스턴스의 분리**: 컨셉마다 프로젝트 맵 템플릿(`map/template_*.map`, `map/field_earth.map`)을 원본으로 영구 보존하고, 인게임에서는 `ResourceSpawner:EnsureHuntMap`이 `PortalDestinationDataSet.csv`를 읽어 런타임 인스턴스를 동적으로 생성한다.
  - **웨이포인트 해금 (영지 연결)**: 새 구역에 **최초로 도달하거나 관문 보스를 격파하면**, 그 구역이 **영지 포탈 목록에 추가**되어 다음부터 영지에서 바로 진입할 수 있다(D2 웨이포인트). 해금 상태는 **영속화**한다.
  - **두 진입 경로**: ① 영지 포탈 목록에서 **해금된 구역으로 직행**, ② 현재 구역의 **연결 포탈로 인접한 더 깊은 구역으로 도보 진행**.
- **난이도 곡선 (Gear-Tier Gating)**:
  - **초기 사냥터(Tier 1) 필드 (`hunt01`~`03`)**: 웬만하면 **돌·나무 장비(T1/T2)로도 클리어 가능**한 낮은 난이도. 처음부터 쉽게 드나든다.
  - **Tier 1 관문 보스 (슬라임킹)**: **구리 빌드(Copper, T3)까지 완성해야 뚫을 수 있는** 난이도. → 따라서 **구리 광석/재료는 초기 사냥터 필드에서 드롭**되게 한다. 루프: T1 필드에서 구리 채굴/드롭 → 구리 장비 제작 → T1 보스 격파 → 바위 고원(`rocky01`) 해금.
...
- **히든 사냥터 (Hidden Dungeons — 절대 격리 원칙 ⚖️ 2026-09-22 확정)**:
  - **중앙 단일 포탈(`WarpPopup` / `PortalDestinationDataSet`) 목록에 절대 노출·연결되지 않는다.**
  - 퀘스트 보상이나 히든 수수께끼로 얻는 전용 가구 설치물(예: '비밀의 균열석'), 일회성 차원 두루마리, 또는 특정 필드의 숨겨진 비밀 입구를 통해서만 독점적으로 진입할 수 있는 독립 비밀 구역으로 운영한다.
...
- **단일 통합 목록 포탈 (영지)**: **사냥터로 나가는 포탈은 영지에 둔다.** 기존 `Portal` 가구(이동 설치 가능)를 **목록형으로 확장** — F/상호작용 시 목적지 **목록 팝업**(D2 웨이포인트 메뉴식 `WarpPopup`)을 띄우고, 행을 선택하면 해당 맵으로 워프한다. 목록은 `공동 마을` + 진입 가능한 정규 `사냥터`들을 데이터 주도(`PortalDestinationDataSet`)로 구성한다.
...
- **웨이포인트 해금이 목록을 키운다**: 영지 포탈 목록은 고정이 아니다. **새 사냥터 구역에 최초 도달하거나 관문 보스를 격파하면** 그 목적지가 목록에 추가된다(유저별 영속, §2.2 ③). 즉 진행할수록 영지에서 바로 갈 수 있는 곳이 늘어난다.
- **데이터 주도**: 목적지(맵/표시명/도착좌표/해금타입[waypoint·level·quest·item]/해금값/필요아이템)는 코드 분기 대신 `PortalDestinationDataSet`로 관리해, 사냥터 추가가 **행 추가만으로** 반영되게 한다.
...
### 3.14. 캐릭터 직업(Job) 및 생활 취미(Hobby) 이원화 시스템 (Job & Hobby Progression) ⚖️ 2026-09-11 확정

- **설계 철학**: 전투/모험의 정체성인 **'직업(Job)'**과 평화로운 라이프/생산 활동인 **'취미(Hobby)'**를 엄격히 분리하여 이원화된 성장 체계를 제공한다.
  - **직업 (Job)**:
    - 캐릭터 기본 레벨(`Level`) 및 전투 스킬트리와 직결.
    - 튜토리얼 이후 마을 4대 멘토(마리, 로체, 엘렌, 토리)의 시험 퀘스트를 통해 전직.
    - 미전직 상태는 **`모험가`**, 전직 후 **`트래퍼`**, **`배틀스미스`**, **`알케미스트`**, **`와일드키퍼`** 4대 직업군으로 특화.
    - 게임 시작 화면(슬롯 선택 카드) 및 캐릭터 정보창에서 `Lv.X · [직업명]`으로 캐릭터의 핵심 정체성 표기. (기존 슬롯의 낚시 레벨 표기 전면 제거)
  - **취미 (Hobby)**:
...
> [!IMPORTANT]
> **체크박스의 의미 (2026-08-06 정의)**
> - `[x]` = **설계·구현이 완결됐다**는 뜻이다. 개별 항목의 **Play 확인 여부는 여기서 추적하지 않는다.**
> - `[ ]` = 미착수 · 부분 이행 · 후속 대기.
> - 🔴 **"지금 뭘 해야 하나 / 무엇이 Play 확인 대기인가"의 단일 소스는 [docs/tasks.md](./docs/tasks.md)** 다. 이 트래커는 *설계 관점의 진행 이력*이다.
>
> **T번호 표기**: 2026-08-04까지 운영한 지휘자/구현자 체제의 티켓 식별자다. 신규 발행은 없다. 티켓 원문·보고서 = [docs/agents/](./docs/agents/).
> Phase 20·21은 발행 순서(20 → 21)가 아니라 **번호 순서로 재정렬**했다(2026-08-06).
...
- [x] 개별 자원 엔티티에 피격 시 회전/Scale 미세 진동 흔들림 효과 컴포넌트 장착 ([ResourceReaction.mlua](./RootDesk/MyDesk/MapObjects/Scripts/ResourceReaction.mlua) - Hit Shake 효과 및 플레이어 위치 가림 시 반투명화 알파 오클루전 처리 구현)
...
- **후속 단위 (Phase 12 이후)**:
  - [~] **Tier 1 보스 — ⚖️ 2026-09-15 레이드로 일원화 (밸런싱 미검증)**: 상시 공유 보스 아레나 `hunt04` 는 **철거**됐고, 사냥터 체인은 **hunt01→hunt03 3단계**에서 끝난다. 보스전은 `BossRaidLogic` 의 2단 인스턴스(**대기실 `template_raid_lobby` → 보스방 `template_boss`**)로만 진입한다 — 상시 부활 보스와 레이드로 경로가 이중화돼 있던 문제를 없앤 조치. 선행 웨이포인트 게이트는 `hunt04`→**`hunt03`** 으로 이관했고(hunt04 웨이포인트는 획득 경로가 사라졌다), `SlimeKing.model` 의 `UnlockWaypointId` 는 비웠다. **잔여**: 난이도 4단(EASY~EXTREME) 밸런싱 검증. 상세: [docs/tasks.md 2026-09-15 항목](docs/tasks.md).
...
- [x] **15-C 낚시** (T18 — **Play 검증 PASS 2026-07-13**. ⚠️ **낚시터 배치 설계는 2026-07-25 Phase 21로 대체됨** — "영지 연못/마을/사냥터 물가 3곳 픽스처" → **"마을만 픽스처 유지 / 그 외는 물 타일 인접 판정"**. 아래 원문은 이력 보존용): 낚싯대(`ToolType=rod`) + 낚시터 픽스처(영지 연못/마을/사냥터 물가 — 타일 지형 불가침) + `FishDataSet`(SpotType별 어종) + 원버튼 타이밍 미니게임(캐스팅→랜덤 대기→`!`→0.8초 윈도우). **→ v2 릴링 개편 코드 완료(T64, 2026-07-18 지휘자 직접 — Play 검증 보류)**: 입질 후 랜덤 미스 폐지(입질=자동 릴링 진입, 실패=텐션 초과 줄 끊김뿐), F/BtnInteract 홀드-릴리즈 릴링(위험 ⚠ 중 릴리즈) + HUD `FishingGauge`(진행 금색/텐션 적색) + 낚시 숙련 레벨(`FishingLevel`/`FishingXp` 영속 — 어종 풀 개방·텐션 완화, `FishDataSet.Difficulty/MinFishingLevel/FishingXp` 컬럼 + 신규 `FishingDifficultyDataSet` 티어 테이블).
  - ⚖️ **릴링 밸런스 계약 (2026-08-06 확정)**: **"⚠ 표시를 무시하고 계속 홀드하면 반드시 줄이 끊긴다"** 가 이 미니게임의 핵심 계약이다. 성립 조건은 `TensionRisePerSec × TensionReliefMinMult × DangerDurationMin ≥ GaugeMax` — 난이도 수치를 조정할 때 이 부등식을 깨뜨리면 위험 표시가 장식이 되고 해당 티어는 무조건 성공한다(2026-08-06 실제 사고: 티어 1 실패율 0%). `FishingSpot.WarnIfDangerToothless()` 가 런타임에 자가검사해 `[FISHING][BALANCE]` 경고를 남긴다. 수치 단일 소스 = `FishingDifficultyDataSet.csv`.
  - 목표 체감: 티어 1 약 7초·손 떼기 2회 → 티어 5 약 11초·손 떼기 3~4회. 반응이 0.6초로 느리면 티어 1·2는 성공하되 티어 3 이상은 실패하는 커브.
...
- [x] **15-E 목장/가축** (T19 — 코드 완료, Play 개별 확인 대기 → [docs/tasks.md](./docs/tasks.md)): 우리 가구 + 닭(씨앗→달걀)/양(풀→양털), 급여→생산 타이머(타임스탬프 환산, 오프라인 포함), 우리 반경 배회(전투 없음 — 영지 평화 원칙 유지).
...
- [x] **15-H 펫 동반자** (T23 — 코드 완료, Play 개별 확인 대기 → [docs/tasks.md](./docs/tasks.md)): 펫 소환 아이템(`UsePetId`) → 추종 + 반경 내 자동 줍기(자석 파이프라인·PickupGrace 규칙 재사용). 희귀 펫은 보스 드롭/업적 보상으로 후속.
...
- [x] **16-F 스킬 QoL 재작업** (T69·T70·T71 — **T70·T71 완료(런타임 검증 포함), T69만 Play 개별 확인 대기** | ⚖️ 2026-07-18 제작자 Play 버그, 배치 K + 지휘자 직접): ① **장착 영속화(T69)** — T45가 장착=세션 값으로 설계(세이브 캡처·필드·복원 전무) → ⚖️ 설계 변경: `equippedSkills` 영속(선캡처, 규칙 9). Play 확인 대기. ② **시전 모션(T70)** — 트리거 자체가 미구현이었음 → `CastAction` 컬럼 + MineState `ActionStateChangedEvent` 미러(런타임 재생 확인). ③ **이펙트 미표시 진범(T71, 지휘자 런타임 격리 실험)** — T46부터 `PlayEffect` **instigator=nil**이 클라 생성 조용한 실패(serial=0)의 원인(서버는 nil 관대 통과 → "서버만 성공" 오진 유발). `self.Entity`로 수정, 시전 경로 클라 serial>0 검증 완료 → **handoff §1.2 규칙 12 신설**. 옵션 딕셔너리·RUID·정렬은 전부 무죄(격리 실험 A~E).
- [x] **16-E 스킬 이펙트 실표시 수리 + 피격 이펙트 + 대시 데미지** (T66 — **T70·T71로 해소 완결**. ⚖️ 2026-07-18 보스 지시, 배치 J ② — 코드 완료 2026-07-18 → Play에서 이펙트 미표시 잔존 → T70 재작업 → **진범 = `PlayEffect` instigator nil(T71, [규칙 12](./docs/pitfalls.md))**. 대시 데미지·피격 훅은 로그상 동작): 제작자 Play "이펙트가 없다" — T46 구현은 존재하나 비주얼 미표시(유력: EffectRUID 무효 또는 톱다운 타일맵 렌더 정렬 — Projectile의 `IgnoreMapLayerCheck`+`EntityLayer` 명시 선례). 진단 후 수리 + 시전 이펙트 방향·전방 오프셋 원작화 + `HitEffectRUID` 컬럼 신설(피격 몬스터 이펙트) + **대시(플래시 점프) 경로 스윕 데미지 부여**(CSV 값 — 기존 3스킬 배율 유지, 전면 리밸런스는 범위 밖). 티켓: handoff §3 T66.
- ⏳ **16-C 직업/전직 (예약 — 티켓 미발행)**: `JobId` 컬럼 예약(공란=초보자 공용) / 직업 선택·전직 퀘스트(quest-achievement 패키지 재사용)·직업별 트리 탭·상위 스킬군. **선행 구조 설계 확정(⚖️ 2026-07-14 보스 지시, 2026-09-09 4대 하이브리드 직업 & 주먹도끼 분기 확정): [skill-tree-plan.md §7·§8](./docs/design/skill-tree-plan.md)** — 주민 4인 연동 하이브리드 직업군(트래퍼/배틀스미스/알케미스트/와일드키퍼) 및 주먹도끼 분기(기간틱 락/쇄석 흩뿌리기, MP 소모 통일), 트래퍼 알뜰한 손놀림(기본공격 투척 포함 100% 무한투척), 배틀스미스 갑주파쇄타(타당 5%, 최대 30% 방깎) 스펙 정립 완료.
...
- [x] **18-A 사운드 파운데이션** (T55 — 코드 완료, Play 개별 확인 대기): 신규 `BGMDataSet`(MapKind별 BGM — home/town/field/boss) + 클라 `BGMManager`(맵 전환 감지, kind 판정=T37 규약 재사용) + `WeatherDataSet.AmbienceSoundRUID`(비 루프 — T21 @Sync 읽기 구독). 음원=msw-search 공식 리소스. 볼륨 설정 UI는 후속.
- [x] **18-B 마을 NPC 생활감** (T56 — **Play PASS 2026-07-21**. 이름표 버그는 T80에서 수정): 주민 2인(촌장/낚시꾼) + `DialogDataSet`(TimeBand/WeatherId/Weight 필터 — 대사 추가=CSV 행) + F 대화 머리 위 말풍선(+선택: 자동 혼잣말). R1: dialog 패키지 확인 우선. 기존 `MerchantInteract` 상호작용 선례 미러.
...
- [ ] **19-D 마을 생활 소품** (T75 — **모델 11종 유지 · 맵 배치 철회 2026-08-04** | Play 보류): P1~P11 `.model`은 남김. 제작자 피드백(디자인·배치·충돌)으로 `town.map` 인스턴스 43개 제거. 재배치는 후속.
...
- [x] **20-J 콜라이더 판정 스케일 미반영 수정** (T101 — ⚖️ 2026-08-01 제작자 Play "조정했는데도 통과", **지휘자 직접**): 엔진 실물 콜라이더는 `BoxSize × Transform.Scale`인데 `ObstacleQuery.GetColliderAABB`가 원본값을 그대로 써서, 후보 수집(실물 콜라이더)과 침투 판정(축소 박스)이 어긋났다 → **`Big Stone1`(Scale 2) 1/2 · `Tree1`(Scale 1.5) 2/3 크기로 판정**. `RenderLayers.ComputeYOrderForEntity`의 접지선에도 동일 결함(Big Stone1 기준선 1.0유닛 위) → 함께 수정. **Maker 박스 조정으로는 상쇄 불가**(실물도 같이 커짐)라 제작자 자력 해결이 불가능한 종류였다. 영향 대상은 `Scale≠1` AND 박스 보유 = **2종뿐**이라 회귀 위험 0. refresh **Error=0 / Warning 17(baseline 유지)**. 런타임은 제작자 Play.
  - 🔴 **부수 성과 — `ModelBuilder` 커버리지 갭 발견(§1.2 규칙 13)**: `maplestorymapobject$` 계열 모델은 본체가 셸 한 단계 안쪽이라 `read()`가 **에러 없이 빈 모델**을 반환한다. 이 갭이 T96 실측표(자원 4종 오기재) → T100 범위 → T101 1차 영향범위 집계까지 연쇄 오염시켰다.
- [x] **20-I 가구 Trigger 부여 + Trigger 전수 재감사** (T100 — **코드 완료 · 지휘자 검수 통과 2026-08-04** | Play 보류): 가구 통행 차단 미작동 확정 → 가구 6종 Trigger(IsPassive) 부여. 전수표 보유 32/미보유 34. T75 회피책 확정. (원 발행 배경: T96 (C)·규칙 13 갭으로 자원은 대상 아님)
- [x] **20-G′ Y정렬 기준점 = 접지선 통일** (T95 — **Play PASS 2026-07-28** | ⚖️ 2026-07-28 제작자 Play 피드백, T89 후속): "건물 기준 Y축 아래 유닛은 앞, 위 유닛은 뒤로 깔려야 자연스럽다". 지휘자 진단 = **방향 규칙은 이미 옳고, 어긋난 것은 비교점**. T89가 전 대상을 `Transform.y` 한 점으로 비교하는데 실제 접지선(`ColliderOffset.y − BoxSize.y/2`)은 **대상마다 0.75~2.25유닛 제각각**(상점 −2.00 / 대장간 −1.90 / 버섯집 −1.65 / 연못 −2.25 / 주민 −0.75) → 큰 건물일수록 실제보다 남쪽으로 취급돼 전환선이 어긋남. ⚖️ 확정 = ① **접지선으로 전 대상 통일** ② **동률 시 유닛이 항상 이김**(캐릭터가 가려지는 사고 방지 우선). ⚖️ **아바타 `Transform` = 발밑 확정**(2026-07-28) → 아바타는 보정 0, Trigger 산출 경로 태우지 않음. 해법 = `ComputeYOrderForEntity`가 **T81 Trigger 박스에서 접지선을 자동 산출**(모델 수작업 0) + `UnitTieBias`. ⚠️ 전수 실측 결과 **45모델 중 Trigger 보유 22 / 미보유 23**(자원·가구·몬스터·동물 계열이 미보유, 단 `Big Stone1/2`만 예외적으로 보유 — 원인 미확인) → 미보유분 처리는 ⚖️ **2026-08-01 보스 확정 — T96 (C) 오브젝트 계열 전면 부여**(자원 6 + 가구 6 + 신규 소품 11; 몬스터·동물·펫은 Transform=발밑이라 제외) → 실행 = **T100**(→ 20-I). → **Trigger 박스가 차단·상호작용·페이드·정렬의 단일 데이터 소스**가 되어 2026-07-25 결정과 정합.
- [x] **20-G 마을 Y축 렌더 정렬 정합** (T89 — **Play PASS 2026-07-28**(T95·T97과 일괄, "깜박임 없음") | ⚖️ 2026-07-25 제작자 Play 버그): "y축 위치에 따라 정렬되어야 하는데 유저도 주민도 정렬이 안 된다". 지휘자 실사 = Y정렬이 **자원·몬스터·가구·건물(T83)에만** 적용됐고 **플레이어 아바타는 엔진 기본 OIL(≈4), NPC 7기는 12 고정, 고양이 2, 낚시터는 SortingLayer 미설정** → 건물 OIL 8,500~11,500 대비 플레이어가 항상 뒤 / NPC가 항상 플레이어 앞 / 주민끼리는 전원 동률. 추가로 **기준 반경 불일치**(`WalkBehindFade.SortRadius=100` vs `ResourceSpawner.MapRadius=30` → 같은 y에서 OIL 7,000 차이). 해법 = `RenderLayers`에 단일 `ComputeYOrder` 규약 신설 + 신규 `YSortSprite` 컴포넌트 + 플레이어 아바타 Y정렬(⚠️ `OrderInLayer`는 `@Sync`가 아니라 각 클라가 전 플레이어를 계산해야 함) + 발밑 기준 오프셋. T83 §5가 예고했던 후속 항목의 실현.
- [x] **20-F 위생·정합 배치 N** (T84→T85→T86→T87 — ⚖️ 2026-07-25 보스 결정. **전량 코드 완료·커밋 `ce13617`, refresh Error=0** | Play 개별 확인 대기): ① **T84** 정적 NPC 7기 `RigidbodyComponent` 청산 — RectTile 맵에 MapleTile용 Body(8대 핵심규칙 1 위반), 정적 엔티티는 Body 불요(platform.md §8.5)이고 차단은 T81 Trigger가 담당하므로 **교체가 아니라 제거** ② **T85** map01 낚시터 복구(위 19-X) ③ **T86** `LWA-4012` Warning 79건 청소 — 모델에 프로퍼티 기본값 미명시(WalkBehindFade 44 / VillagerDialog 24)로 baseline 25→85 증가, 실 Error가 노이즈에 묻힘 ④ **T87** `RootDesk/MyDesk/` 최상위 방치 자산 9개 정리(디렉터리 2단계 규칙 위반 — `.png`/`.pxg` 작업 원본이 Maker 스캔 경로에 존재).
...
- [x] **21-A′ map01 물가 L2 프린지** (T98 — **코드 완료 2026-08-04** | Play 보류): 제작자 L1 Water 61셀 페인팅 후 범용 `fix_water_fringe.cjs`로 L2 **40셀** 프린지 · 범위 밖 diff 0. ⚠️ 1차 커밋 `jsonString` 문자열 붕괴(LEA-3015) → 미푸시 rewrite `d7b9479` 복구 · **규칙 16**. build Error=0은 `.map` 구조 붕괴를 못 잡음.
  - ✅ **인정분(T90)**: `wall.tileset`의 `Water` 타일 정의 + 판정 함수.
  - ⚖️ **2026-07-28 보스 결정 — 물 페인팅은 제작자가 Maker에서 직접, 에이전트는 프린지 보정만**(→ **T98**).
  - ⏳ town/field/boss Water 페인팅·프린지는 후속(스크립트 범용 재사용).
- [ ] **21-A 물 타일 기반** (T90 — 🔴 **부분 이행 → 인정분만 유지, 배치분은 T98**): `wall.tileset` `Water` + `IsWaterTileName` + 통행 불가 + 스폰 억제 + 미니맵. **실배치는 T98**.
- [x] **21-B 낚시 = 물 타일 인접 판정** (T91 — 코드 완료 · refresh Error=0 | ⏳ **물 미배치로 사냥터 경로 미검증**, 영지는 21-C 파기로 자체 검증 가능): 조준 셀이 물이면 F로 캐스팅. **마을은 픽스처 유지 / 영지·사냥터 픽스처 제거** — 판정 우선순위 = 픽스처 `SpotType` → 없으면 맵 기반. 맵 종류는 T37 mapKind 규약 재사용, 어종은 CSV 행 추가. ⚠️ **T85(영지 낚시터 복구, 커밋 `ce13617`)를 되돌리는 티켓** — 같은 날 설계 변경에 따른 것으로 회귀가 아니다.
- [x] **21-C 영지 물 파기** (T92 — 코드 완료 · refresh Error=0 | Play 개별 확인 대기): 지형 편집에 `dig_water`/`fill_water` action 추가(영지 전용 게이트). 되메우기 필수, **자기 자신이 갇히는 배치 방지 가드** 필수(물이 통행 불가이므로).
  - ⚖️ **2026-08-06 결정 변경 — 물 편집은 전용 도구(`Water Spade`)로 분리**. 2026-07-25의 "CSV 행 추가 없이 기존 `Shovel` 재사용" 결정을 뒤집는다.
    - **사유**: 삽 하나가 길 파기와 물 파기를 겸하면 **둘을 고를 수단이 없다.** T92 구현이 실제로 `digPath`를 물로 덮어써 Phase 14-A의 길 파기가 통째로 사라졌다(2026-08-06 제작자 신고 → 회귀 수정).
    - **결과**: `Shovel` = `digPath` 전용 복귀 / `Hoe` = `digHole` / **`Water Spade` = `dig_water`만**(되메움은 별도 도구 — 2026-08-07 토글 폐기). 제작 = Wood 2 + Stone 2, Tier 1.
    - 🔴 **`PlayerInventory`의 물 토글 조건에 `digPath`를 다시 넣지 말 것** — 넣는 순간 길 파기가 도달 불가능해진다. 해당 위치에 경고 주석 상주.
    - ⚖️ **2026-08-07**: Water Spade가 물 위에서 되메움까지 겸하면 실수로 메워지는 사고가 나 토글을 제거. `fill_water` 액션·코드는 복구 도구용으로 유지. 임시 `G`=`forceGrass` 3×3은 지형 델타 영속.
- [x] **21-D 몬스터 접근 차단 장치** (T93 — **Play PASS 2026-07-28**, 전 항목 확인): 설치 반경 내 몬스터 회피(히스테리시스로 경계 떨림 방지, 활성 장치 캐시, **보스는 예외**). 반경·지속시간 CSV. **사냥터·보스맵 전용** — ⚖️ 영지 평화 원칙상 영지에서는 무의미.

---

## 5. 향후 개발 마일스톤 (Upcoming Milestones)

### Phase 22: 개인 영지 '섬(Island)' 컨셉 전환 & 수역 테두리 구축 (✅ 2026-09-02 완료)
> **배경 및 의도**: 기존 영지의 인공적이고 답답한 돌벽(`Wall`) 외곽을 전면 철거하고, 푸른 바다/호수 위에 떠 있는 **아늑한 나만의 '섬(Island)'** 컨셉으로 영지를 탈바꿈합니다.

- **22-A 외곽 수역 테두리 교체 (`map01.map` 외곽 전면 개편)**: ✅ 완료
  - 기존 4면의 `Wall` 타일을 제거하고, 외곽 3~5칸 폭의 영역을 `RectTileMap0` (L0 Water 수면)과 `RectTileMap6` (L6 WetRim 수변 림/윤슬)로 포위.
  - 섬의 가장자리 해변은 모래톱/젖은 흙/잔디 프린지 타일(`GrassSand_*`, `WetRim_*`, `Water*`)로 자연스럽고 유려한 해안선 형성.
- **22-B 보이지 않는 경계벽 (Boundary Collider) & 영지 가드**: ✅ 완료
  - 플레이어가 물 밖(맵 외곽)으로 무한정 걸어나가는 것을 방지하기 위해 외곽 수역 경계에 `KinematicbodyComponent` 기반 보이지 않는 충돌 벽 배치.
  - 영지 밖 좌표 가드(`-27 ~ +27`)와 연동하여 안전한 플레이 영역 확보.
- **22-C 선착장 / 부두(Dock) & 출항 포탈 연출**: ✅ 완료
  - 섬의 한쪽(남쪽)에 나무 데크/선착장(Dock)과 나룻배(`Boat`) 가구 오브젝트 배치.
  - 선착장의 포탈 및 중앙 마법 포탈(`Portal`)을 통해 공동 마을(`town`)로 출항/워프하는 세계관 연출 강화.

---

### Phase 23: 커뮤니티 & 소셜 인프라 (채팅, 친구, 영지 초대/방문) (🔨 진행 중 — ⚖️ 2026-08-29 확정)
> **배경 및 의도**: 현재 마을에 다른 유저가 접속해 있어도 상호작용 수단이 전무한 상태입니다. 메이플/스타듀밸리 특유의 따뜻하고 생동감 넘치는 유저 간 교류 인프라를 구축합니다.

- **23-A 채팅 시스템 (Chat System & 말풍선)**: 🔨 진행 중 (채팅 UI/통신 완료, 말풍선 잔여 과제)
  - **채팅 탭 분리 & UI 컨트롤러**: ✅ 완료 (HUD 좌하단 반투명 채팅 패널, [전체] / [귓속말] / [시스템] 탭, 최근 50개 로그 뷰어, 터치/클릭 입력 및 `Enter`/`T` 단축키 포커스 연동).
  - **채팅 백엔드 라우터 (`ChatManager.mlua`)**: ✅ 완료 (맵 내 브로드캐스트, 1:1 귓속말 및 `/w 닉네임` 명령어 파싱, 시스템 알림, 0.5초 도배 쿨다운).
  - **아바타 머리 위 말풍선 (Chat Balloon)**: ⚠️ *추후 작업 예정 (별도 고도화 체크)*
- **23-B 유저 인터랙션 메뉴 (User Interaction Menu)**: (예정)
  - 마을/필드에서 타 플레이어 클릭/터치 시 컨텍스트 팝업 메뉴 노출:
    - `[프로필/장비 보기]` · `[친구 추가]` · `[내 영지 초대]` · `[귓속말]` · `[1:1 거래 신청 (장기)]`.
- **23-C 친구 시스템 (Friend System)**: (예정)
  - `FriendListPopup`: 친구 목록, 온라인/오프라인 상태, 현재 위치(마을 / 영지 / 사냥터) 실시간 표기.
  - 친구 요청/수락/거절/삭제 및 접속 알림 토스트.
- **23-D 개인 영지 초대 & 게스트 방문 시스템 (Guest Estate Visit)**: (예정)
  - 유저 A가 유저 B에게 [영지 초대] 발송 ➔ B에게 "A님이 자신의 영지(초록 섬)로 초대했습니다" 팝업 ➔ 수락 시 `Home_<A_UserId>` 인스턴스로 워프.
  - **게스트 권한 가드**: 손님은 타일 파기(삽/호미), 자원 벌목/채광, 가구 설치/철거 차단 (영지 훼손 원천 방지).
  - **게스트 허용 상호작용**: 대화, 낚시(손맛 공유), 의자/침대 휴식, 방명록 작성 및 '좋아요' 추천.

---

### Phase 24: 메이플 스타일 UI 고도화 & UX 개선 (🔨 진행 중 — ⚖️ 2026-08-29 확정)
> **배경 및 의도**: [docs/design-policy.md](./docs/design-policy.md)에 명시된 비주얼 아이덴티티(메이플 클래식 감성 + 현대적 반응형 UX)를 전면 적용하여 시각적 완성도와 조작 편의성을 극대화합니다.

- **24-A 윈도우 크롬 & 비주얼 테마 통일**: 🔨 진행 중 (팝업 12종 Title 컴포넌트 마이그레이션 완료)
  - 인벤토리, 캐릭터 정보, 제작창, 퀘스트창, 설정창 등 모든 팝업에 통일된 메이플 9-슬라이스 원목 프레임 및 황금 리벳 헤더 적용.
  - 팝업 12종 제목(Title) 텍스트 레거시 `TextComponent` ➔ `TextGUIRendererComponent` 전면 교체 완료 (크롬 규격/시각값 100% 보존).
  - 상단 탭(Tab), 닫기(X), 수량 스테퍼(`-`/`+`) 디자인 일관성 확보.
- **24-B HUD 레이아웃 정돈 & 조작 반응성 강화**:
  - 좌상단(프로필/레벨/HP/스태미나 바) · 우상단(미니맵/위치/날씨) · 하단(10칸 퀵슬롯/스킬바) · 우하단(메인 메뉴 버튼군) 구조 확립.
  - 버튼 마우스 호버(Hover) 시 ColorTint/스케일 펄스 연출 및 클릭 SFX 정합.
- **24-C 아이템 툴팁 & 장비 비교 인포메이션**:
  - 아이템 등급별 테두리 색상(일반/희귀/에픽/레전더리) 및 상세 설명.
  - 장비 비교 툴팁 (현재 장착 장비와 비교하여 공격력/채집효율 증감 수치 `+2(초록)` / `-1(빨강)` 표시).
- **24-D PC / 모바일 반응형 조작 UI 대응**:
  - PC: 키보드 단축키 배지(`[C] 제작`, `[I] 가방`, `[U] 정보`, `[Tab] 퀘스트`) 표시.
  - 모바일: 터치 친화적 가상 액션 버튼군 및 조이스틱 최적화.

---

### Phase 25: 퀘스트 라인 & 몬스터 생태계/패턴 확장 (⚖️ 2026-08-29 확정)
> **배경 및 의도**: 플레이어에게 명확한 플레이 동기와 성장 목표를 제시하고, 사냥터의 다채로운 전투 경험을 제공합니다.

- **25-A 메인 퀘스트 라인 (온보딩 ~ 사냥터 개방)**:
  1. *표류와 정착*: 나뭇가지/조약돌 줍기 ➔ 손도끼 제작 ➔ 첫 벌목/채광
  2. *나만의 보금자리*: 나무 바닥/벽 설치 ➔ 안락한 침대 제작 및 수면 회복
  3. *바다 건너 마을로*: 선착장 보트 수리 ➔ 공동 마을(Town) 첫 방문
  4. *새로운 인연*: 촌장/상인/대장장이와 인사 ➔ 낚싯대 수령 및 낚시 입문
  5. *연구소의 비밀*: 슬라임 방울 납품 ➔ 첫 가공 기술(연구) 해금
  6. *원정의 시작*: 사냥터 1구역(초원) 포탈 개방 및 초보 몬스터 퇴치
- **25-B 일일 의뢰 (마을 퀘스트 게시판)**:
  - 매일 갱신되는 3종의 의뢰 (어종 납품, 목재/석재 조달, 몬스터 n마리 처치) ➔ 보상: 골드, 연구 포인트, 전용 칭호.
- **25-C 사냥터 티어별 몬스터 생태계 & 패턴 설계**:
  - **Tier 1: 여명의 숲 (Greenwood Plains) - 초급**:
    - *달팽이 (Snail)*: 느린 이동, 비선공, 초보자용 껍질/고기 드롭.
    - *초록슬라임 (Slime)*: 통통 튀는 이동, 근접 접촉 데미지, 끈적한 방울 드롭.
    - *주황버섯 & 뿔버섯 (HornMushroom)*: 뿔버섯은 원거리 포자 투사체 발사.
    - *필드 보스: 슬라임킹 (SlimeKing)*: 지진 도약(Leap), 미니언 소환, 젤리 바운스 3단 공격.
  - **Tier 2: 은빛 해변 & 맹그로브 늪지 (Silver Coast & Mangrove Swamp) - 중급**:
    - *모래 꽃게 (Sand Crab)*: 단단한 껍질(높은 방어력), 옆걸음 돌진, 집게 공격.
    - *늪지 옥토퍼스 (Octopus)*: 먹물 투사체(시야 방해/슬로우 디버프), 촉수 휘두르기.
    - *와일드 보어 (Wild Boar)*: 붉은 안광 윈드업 후 초고속 직선 돌진, 넉백 유발.
    - *필드 보스: 킹크랩 (King Crab)*: 모래 폭풍 소환, 집게 방어 태세(데미지 반사), 거대 집게 파쇄.
  - **Tier 3: 잊혀진 바위 유적 (Rocky Ruins) - 상급**:
