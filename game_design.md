...
- **22-B 보이지 않는 경계벽 (Boundary Collider) & 영지 가드**:
...
- **22-C 선착장 / 부두(Dock) & 출항 포탈 연출**:
  - 섬의 한쪽(남쪽 또는 동쪽)에 나무 데크/선착장(Dock)과 뗏목/보트 오브젝트 배치.
  - 선착장의 포탈을 통해 공동 마을(`town`)이나 공용 사냥터로 나아가는 "출항" 느낌의 세계관 연출 강화.

---

### Phase 23: 커뮤니티 & 소셜 인프라 (채팅, 친구, 영지 초대/방문) (⚖️ 2026-08-29 확정)
> **배경 및 의도**: 현재 마을에 다른 유저가 접속해 있어도 상호작용 수단이 전무한 상태입니다. 메이플/스타듀밸리 특유의 따뜻하고 생동감 넘치는 유저 간 교류 인프라를 구축합니다.

- **23-A 채팅 시스템 (Chat System & 말풍선)**:
  - **채팅 탭 분리**: [전체/마을] (현재 맵 브로드캐스트), [귓속말] (특정 유저 1:1), [시스템] (획득/레벨업 알림).
  - **아바타 말풍선 (Chat Bubble)**: 메시지 전송 시 캐릭터 머리 위에 메이플 클래식 스타일의 둥근 말풍선 팝업 (5초 후 자연스러운 페이드아웃).
  - 비속어/도배 방지 쿨다운 및 이모티콘 지원.
- **23-B 유저 인터랙션 메뉴 (User Interaction Menu)**:
  - 마을/필드에서 타 플레이어 클릭/터치 시 컨텍스트 팝업 메뉴 노출:
    - `[프로필/장비 보기]` · `[친구 추가]` · `[내 영지 초대]` · `[귓속말]` · `[1:1 거래 신청 (장기)]`.
- **23-C 친구 시스템 (Friend System)**:
  - `FriendListPopup`: 친구 목록, 온라인/오프라인 상태, 현재 위치(마을 / 영지 / 사냥터) 실시간 표기.
  - 친구 요청/수락/거절/삭제 및 접속 알림 토스트.
- **23-D 개인 영지 초대 & 게스트 방문 시스템 (Guest Estate Visit)**:
