...
1. **지상(잔디 vs 흙길)과의 완전한 독립**:
   - 잔디밭에 흙길을 파거나 밭을 갈 때 물과의 간섭/충돌을 고려할 필요가 전혀 없음.
2. **단일 2진 판정 (물 vs 땅)**:
   - 물을 파고 메울 때 "이웃이 잔디인지 흙길인지" 구분하지 않고, 오직 **"이웃이 물인지 땅인지"** 1축 마스크(`0`~`15`) 하나로 `Water*` 타일과 `WetRim*` 타일을 결정.
3. **자연스러운 믹스 연출**:
   - 잔디밭에 물을 파면 ➔ 초원 연못
   - 흙길에 물을 파면 ➔ 흙빛 해변/부둣가
   - 둘이 만나는 코너에서도 투명 영역 아래로 잔디와 흙이 그대로 비쳐서 **추가 타일 없이 100% 자연스럽게 합성**.
