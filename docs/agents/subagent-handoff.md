...
> 🧊 **동결 (2026-08-06)** — 이 문서는 **더 이상 갱신하지 않는다.** 지휘자/구현자 위임 체제가 솔로 체제로 전환되면서 살아있는 내용은 전부 이관됐다.
>
> | 여기 있던 것 | 지금 보는 곳 |
> |---|---|
> | §1.2 절대 규칙 1~17 | [../pitfalls.md](../pitfalls.md) — **규칙 번호 그대로** |
> | §1.3 타일 스킴 | [../tile-scheme.md](../tile-scheme.md) |
> | §1.4 검증 프로토콜 | [../workflow.md](../workflow.md) §2 |
> | §1.5 상시 디자인 정책 | [../design-policy.md](../design-policy.md) |
> | §3 작업 큐 잔여분 | [../tasks.md](../tasks.md) |
> | §4 보고 형식 · §5 킥오프 프롬프트 | 폐기 |
>
> **남겨둔 이유**: §3의 T4~T103 티켓 스펙(Target/Change/Acceptance)과 지휘자 현황판 시계열이 1차 자료로 남는다. 아카이브 안내 = [README.md](./README.md).

> **용도(당시)**: 상위 에이전트/보스가 하위 에이전트에게 작업을 위임할 때 이 문서를 그대로 전달한다.
...
15. 🔴 **`.ui` 엔티티의 프리팹 타입(`origin.entry_id`/`modelId`)이 컴포넌트를 강제한다 (2026-08-01 신설 — 규칙 11 4차 사고의 진짜 원인)**: `.ui` 엔티티는 프리팹 모델 인스턴스이고, **모델이 정의한 컴포넌트는 인스턴스에서 지워도 Maker가 로드/저장 때 복원**한다(Maker UI: *"모델에서 정의한 컴포넌트라서 제거할 수 없다"*). 따라서 **컴포넌트를 지우는 건 증상 치료이고 무한 재발**한다.
    - **사고 실례**: `FurnacePopup`이 `entry_id="UIGroup"` / `modelId="uigroup"` 이었다. 빌더의 `group()`이 심는 타입으로(`msw_ui_builder.cjs` 1585행) `UITransform + UIGroupComponent + CanvasGroupComponent`를 함께 정의한다. **T79·T88이 `UIGroupComponent`만 두 번 지웠고 두 번 다 되살아났다**(`9b09dc3`→`ce13617`, `d656aa5`→2026-08-01). 형제 팝업 11종은 전부 `uiempty`라 이런 일이 없었다.
...
> - **🔴 2026-08-01 지휘자 직접 — T102: 규칙 11 "4차 사고"의 근본 원인 규명·제거**
>   - **제작자 관찰이 결정적이었다**: "리프레시 이후에도 **다른 UI를 수정할 때마다** 다시 붙는다" + Maker의 **"모델에서 정의한 컴포넌트라서 제거할 수 없다"**. 이건 스테일 저장이 아니라는 뜻이다.
>   - **진짜 원인**: `FurnacePopup`이 `modelId="uigroup"`(빌더 `group()`이 심는 타입) 인스턴스였고, 그 모델이 `UIGroupComponent`·`CanvasGroupComponent`를 **정의**한다. 인스턴스에서 지워도 Maker가 복원 → **T79·T88은 증상만 지운 것이라 재발이 예정돼 있었다**(독립 재발 2회). 형제 팝업 11종은 전부 `uiempty`라 무사고.
>   - **수정**: 통째 재구성 불요 — `empty()` 재호출로 **타입만 교체**(`uiempty`). UUID·자식 14개·트랜스폼·메타 전부 보존, 형제 11종과 형태 완전 일치, `ui_lint` 0 error.
>   - ⚠️ **지휘자 1차 가설 오류 기록**: "CanvasGroup이 UIGroup을 끌고 온다"고 봤으나 **틀렸다**. 둘이 늘 세트인 건 인과가 아니라 같은 모델이 정의해서였다. 상관관계가 깨끗해도(3/3 대 0/715) 메커니즘 확인 전엔 단정하지 말 것.
>   - **판정은 1사이클**: 제작자가 다른 UI를 수정·저장한 뒤에도 재부착이 없으면 확정. 재발하면 규칙 15 판정 절차부터 다시.
>   - 🔴 **규칙 11 3차 사고 기록도 재검토 대상**: `ce13617` 건 역시 같은 원인일 가능성이 높다 — "스테일 저장" 분류가 과잉이었을 수 있다.
> - **🧭 2026-08-04 지휘자 — 단일 에이전트 순차 배치 (배치 O) 발행**
>   - **T102 제작자 확인 완료** ("정상적으로 작동"). 규칙 11 계열 재발 고리는 끊긴 것으로 본다 — 단 최종 판정은 다른 UI 수정·저장 1사이클 후.
...
### T88. [⚠️ 이력 정정 (2026-08-01) — **증상만 제거해 재발함**. 근본 원인은 프리팹 타입이며 **T102**에서 해소 | 코드 완료 2026-07-28 | ui_lint error=0] T79 L029 수정 원복 — Maker 스테일 저장 3차 사고 복구 (규칙 11)

> ⚠️ **2026-08-01 정정**: T79·T88이 지운 `UIGroupComponent`는 `FurnacePopup`의 **프리팹 모델(`modelId="uigroup"`)이 정의**하는 컴포넌트였다. 인스턴스에서 지워도 Maker가 복원하므로 두 티켓 모두 **원리상 재발이 예정돼 있었다**(T88 이후 2026-08-01에 4차 재발). 이 건은 "스테일 저장(규칙 11)"이 아니라 **§1.2 규칙 15**의 사례로 재분류한다. 근본 수정 = **T102**(타입을 `uiempty`로 교체).
...
>   - 단 **`scripts/fix_water_fringe.cjs`는 맵 경로를 인자로 받는 범용 스크립트**로 작성할 것. 나머지 3맵은 제작자가 물을 칠한 뒤 **같은 스크립트를 재실행**하면 끝나야 한다. `map01` 전용 하드코딩 금지(R3).
> - **`Name44` 오배치는 착수 차단 사유가 아니다.** L1 `(3, 6)`의 `Name44`는 타일 페인팅 갭이라 제작자만 고칠 수 있지만, 이 티켓의 작업은 **L2 프린지**다. 스크립트는 그 셀을 **감지해 경고 로그로 보고하고 건너뛴다** — 실패로 처리하지 말 것. 제작자 복구는 병행된다.
> - **잔여 실작업 = Change ①ⓑ(프린지)뿐**: 병렬 세션 실측으로 ⓐ(물 셀 L2 비우기)는 **61/61 이미 홀**이라 할 일이 0이다. 물 8이웃 **86셀이 전부 `FullGrass`** 인 것을 §1.3 문법대로 방향 에지·`Grass*Corner`로 바꾸는 것이 전부다.

> **🔎 2026-08-03 실측 (에이전트 — `MapBuilder` 읽기 + `wall.tileset` 파싱)** — 제작자 물 페인팅 결과 검수:
>
> | 항목 | 실측값 | 판정 |
> |---|---|---|
> | 물 배치 레이어 | `map01` **L1 `RectTileMap`에 `Water` 61셀** (x[18..26] y[-25..-17]) | ✅ §1.3 스킴·T90 설계와 일치 |
> | 물 셀의 L2(`RectTileMap2`) | **61/61 전부 홀** (잔디 덮임 0) | ✅ Change ①ⓐ 이미 충족 — 비울 셀 0 |
> | `Water` 통행 판정 | 타일셋 `IsCollidable=true` (인덱스 44) | ✅ Acceptance ⑥ 구조적 충족 |
> | **물가 프린지** | 물 8이웃 **86셀 전부 `FullGrass`** — 방향 에지·`Grass*Corner` **0개** | ❌ **Change ①ⓑ 미착수 = 이 티켓의 실작업 전부** |
> | 타 맵 물 | `town`·`template_field`·`template_boss` **`Water` 0셀** | ⚠️ T91(맵별 어종) 성립 불가 |
>
> - 🔴 **선행 수정 (제작자 전용)**: `map01` **L1 `(3, 6)` 셀에 `Name44` 타일이 오배치**돼 있다. L1은 `Soil`/`Water`만 허용. 원인 추정 = 타일셋에서 **`Water`는 인덱스 44**인데 **`Name44`라는 이름의 슬롯은 인덱스 36**이라, "물=44"로 알고 이름이 `Name44`인 슬롯을 고른 혼동. 타일 페인팅은 [`builder-protocol.md`](../../.claude/skills/msw-general/references/builder-protocol.md) §1.6 커버리지 갭이라 **에이전트가 고칠 수단이 없다** → Maker에서 `Soil`로 복구할 것.
> - 🧭 **범위 결정 대기**: 프린지 보정을 `map01` 단독으로 할지, 나머지 3맵 물 페인팅을 기다려 일괄로 할지 미정. 제작자 답변 후 착수.
...
### T102. [✅ 완료 — **제작자 확인 2026-08-04 "정상적으로 작동"** | 지휘자 직접 | ui_lint 0 error / 엔티티 341 유지 · UUID·자식·트랜스폼 보존] `FurnacePopup` 프리팹 타입 교체 — 규칙 11 4차 사고의 근본 원인 제거 (⚖️ 2026-08-01 제작자 관찰)

- **배경(제작자)**: "리프레시 완료 이후(정리된 ui)에서 **다른 UI를 수정할 때마다 다시 붙는** 것 같다." + Maker 메시지 **"모델에서 정의한 컴포넌트라서 제거할 수 없다"**.
- **🧭 지휘자 진단 — T79·T88은 증상만 지웠다**: `FurnacePopup`의 `origin.entry_id="UIGroup"` / `modelId="uigroup"`. 빌더 `group()`이 심는 타입이며(`msw_ui_builder.cjs` 1585행) `UITransform + UIGroupComponent + CanvasGroupComponent`를 **모델 차원에서 정의**한다. 인스턴스에서 컴포넌트만 지우면 Maker가 모델 정의를 복원하므로 **원리상 무한 재발**이다.
  - **이력 근거**: 2026-06-22~07-16 **21개 커밋 전부** `UIGroup+CanvasGroup` 세트 보유(화로 시스템 저작 당시 `group()`으로 생성). T79(`9b09dc3`) 제거 → `ce13617` 복귀 → T88(`d656aa5`) 제거 → 2026-08-01 복귀. **독립 재발 2회.**
  - **대조군 근거**: 두 `.ui`의 전체 718개 엔티티 중 `uigroup` 타입은 루트 2개(정상) + `FurnacePopup` 1개뿐. 형제 팝업 11종은 전부 `uiempty`라 단 한 번도 재발이 없었다.
  - ⚠️ 지휘자 1차 가설("`CanvasGroupComponent`가 `UIGroupComponent`를 끌고 온다")은 **틀렸다** — 둘이 항상 세트인 건 인과가 아니라 **같은 모델이 정의하기 때문**이었다. 제작자의 Maker 메시지가 방향을 잡아줬다.
- **Target**: `ui/PopupGroup.ui` (`FurnacePopup` 루트 엔티티만)
- **Change**: ① `b.empty("FurnacePopup", {enable:false})` 로 타입 교체(`entry_id`→`UIEmpty` / `modelId`→`uiempty`) ② 원본 `UITransformComponent` 를 `upsertComponent`로 그대로 복원 ③ `script.UIFurnaceController` 재부착 ④ `enable`/`visible`/`localize`/`displayOrder` 를 `patch()`로 복원 ⑤ `UIGroupComponent`·`CanvasGroupComponent`는 **재부착하지 않음**(후자는 `.mlua` 전체 참조 **0건** — 완전 미사용) ⑥ §1.2 **규칙 15** 신설.
- **검증 결과**: UUID `a434dc9c…` 보존 · 자식 14개 전부 보존 · 트랜스폼 JSON 완전 동일 · 메타 4종 동일 · 엔티티 341 유지 · `ui_lint` **0 error**(L029 소멸) · **형제 팝업 11종과 형태 완전 일치**.
