...
3. **하드 제약 위반 콘텐츠 금지**: 퀘스트 조건 11종, `CountMode` Action/State, 아이템 영문 `Name` 키, 대사 규칙(40자·이모지 금지)은 [quest-design.md](./quest-design.md) §1이 요약한다. 시스템에 없는 것이 필요하면 콘텐츠에 섞지 말고 **"시스템 제안"으로 분리 표기**한다.
