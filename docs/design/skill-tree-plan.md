...
### 7.3 직업 4종 체계 & 마을 서사 연계 (⚖️ 2026-09-09 확정)

메이플 원작 4대 직업을 단순 복제하지 않고, **은퇴 모험가의 생존·제작과 마을 주민 4인의 전문 기술이 융합된 본작 고유의 하이브리드 직업군**을 도입한다.

| 직업명 (`JobId`) | 멘토 NPC | 컨셉 & 정체성 | 핵심 기믹 및 대표 스킬 |
|---|---|---|---|
| **`trapper`**<br>(트래퍼 / 사냥꾼) | **노점상 마리** | 투척 무기, 함정, 날렵한 기동력과 전리품 특화 | • **알뜰한 손놀림 (Ammo Salvage)**: 투척 스킬뿐만 아니라 향후 추가될 **모든 '투척 아이템'의 기본 공격(Ctrl) 투척에도 100% 연동**. 레벨에 따라 확률 증가, **최종 마스터 시 100% 소모 방지 (무한 투척)**.<br>• **사냥꾼의 덫**: 설치형 속박 및 방어력 감소.<br>• **전리품 감별안**: 몬스터 가죽/고기 등 전리품 추가 드롭 확률. |
| **`battle_smith`**<br>(배틀스미스 / 대장전사) | **대장장이 로체** | 채광과 제련, 중장갑 둔기/도끼 물리 분쇄 특화 | • **갑주 파쇄타 (Armor Breaker)**: 타격당 **5%씩 방어력 감소 디버프 누적, 최대 6스택(30% 방어력 감소)**. 보스 및 단단한 몬스터 파쇄 특화.<br>• **담금질 피부**: 광석 보유 및 방어구 착용 시 고정 피해 감소.<br>• **대지 분쇄타**: 지면 강타 범위 기절. |
| **`alchemist`**<br>(알케미스트 / 연금사) | **연구원 엘렌** | 푸른 불씨, 마나, 슬라임 젤리, 약초 비전 마법 | • **푸른 불씨 폭발**: 응집된 마나 구체 폭발.<br>• **슬라임 산성 포션**: 지면 지속 장판 피해 및 슬로우.<br>• **비전 마나 친화**: 최대 MP 및 MP 자연 회복 속도 대폭 증가.<br>• **매직 클로** (⚖️ 2026-09-15 공용 탭에서 이관): 마법 발톱 투사체. 마법 공격력 의존이라 공용이 아닌 이 직업의 주력 딜링으로 배치. |
| **`wild_keeper`**<br>(와일드키퍼 / 자연수호자) | **헛간지기 토리** | 야생 동물 조련, 넝쿨/자연 방벽, 생태 지원 | • **야생의 호루라기**: 멧돼지/동물 소환 돌격 및 넘어뜨림.<br>• **가시 넝쿨 방벽**: 근접 공격 반사 가시 실드.<br>• **자연의 활력**: 도트 힐 오라 및 영지 동물 호감도/생산 가속. |

- **전직 퀘스트 동선**:
  1. 튜토리얼 101~107 완료 후 마을 입성.
  2. 주민 4인의 기초 서브 퀘스트를 수행하며 각 주민이 주인공의 옛 감각을 눈치챔.
  3. 희망 직업 멘토에게 '전직 시험 퀘스트' 수주 (직업별 고유 과제 + **네 직업 공통 관문**).
  4. 퀘스트 보상 `RewardJobId`로 전직 확정 → 스킬창에 해당 직업 칩 탭 개방.

- **공통 관문 — 슬라임킹의 대형 젤리 (⚖️ 2026-09-14 확정)**: 네 시험(302/312/322/332) 모두 초급 사냥터 보스 **슬라임킹**을 처치해 얻는 **대형 슬라임 젤리(`Large Slime Jelly`) 1개 제출**을 요구한다. 직업 선택과 무관하게 "보스를 한 번은 넘어야 한다"는 통과 의례를 두기 위함이다.
  - 평시 드롭 확률 **3%**. 시험을 수락하고 아직 젤리를 갖고 있지 않은 동안에는 `ItemDropDataSet.GuaranteeQuestIds` 판정으로 **막타 플레이어에게 1회 확정 드롭**되며, 획득 즉시 원래 확률로 복귀한다(별도 세이브 플래그 없음 — 보유 수량이 곧 상태). 상세: [tasks.md 2026-09-14 항목](../tasks.md).
- **전직 보상 무기 (⚖️ 2026-09-14 확정)**: 시험 완료 시 직업 전용 무기를 지급한다 — 트래퍼 `사냥꾼의 투척검`(평타), 배틀스미스 `대장장이의 빅해머`(양손 최고 평타), 알케미스트 `푸른 불씨 완드`(MagicAttack), 와일드키퍼 `네이처 바인`(SkillAttack). 수치·RUID는 [tasks.md 2026-09-14 항목](../tasks.md) 표 참조.

---

## 8. 트리 위상 시각화 & 연계 강화 (⚖️ 2026-07-15 보스 지시, 2026-09-09 확장)
...
### 8.1 공용 탭 확정 위상 & 주먹도끼 분기 체계 (⚖️ 2026-09-09 확정)

- **주먹도끼의 본질**: 자루 달린 금속 도끼가 아닌, **구석기 시대의 뗀석기/돌멩이** 대체재.
- **주먹도끼 진화**: 직렬 단계적이 아닌, 스타일별 **1:2 분기 선택 (Branching)** 구조. 두 분기 모두 **`주먹도끼 1개 + 마나(MP)` 소모로 통일**.

```
[전투 계열]               [이동/투척 계열]               [채집 계열]
파워 스트라이크(1,1)     플래시 점프(1,2)             신속 채집(1,3)
      │ (Lv 3 선행)                                     │
슬래시 블러스트(2,1)     주먹도끼 던지기(2,2)          강력 채집(2,3)
                                  │ (Lv 3 선행)
회복(3,1) ※선행 없음      ┌──────┴──────┐
                         ▼             ▼
                    기간틱 락(3,2)   쇄석 흩뿌리기(3,3)
                  (관통/넉백/지진파) (부채꼴 산탄 폭딜)
```

#### 매직 클로 이관 & 회복 신설 (⚖️ 2026-09-15 확정)

- **매직 클로(`fireball`) → 알케미스트 탭 (2,1)** (선행 `비전 마나 친화` Lv 3, `ManaCost` 12). 사유: 모두가 배우는 공용 스킬치고 연출이 거창하고, **`DamageModel=Magic` 인데 초반 마법 공격력이 사실상 0** 이라 체감이 죽는다. 마법/마나 계열 정체성은 알케미스트(연구원 엘렌)이므로 그쪽으로 옮긴다. `SkillId` 는 세이브 키라 **변경하지 않는다**(§7.1).
- **빈 (2,1) 은 슬래시 블러스트가 올라와 채운다.** 슬래시 블러스트 선행 = **파워 스트라이크**(Lv 3). 🔴 `UISkillTreeController.RefreshLinks` 의 위상 가드가 `부모행 == 자식행 − 1` 이라, 슬래시 블러스트를 (3,1)에 두고 부모만 파워 스트라이크로 바꾸면 **연결선이 그려지지 않고 `link topology skip` 경고만 남는다** — 행을 함께 올려야 한다.
- **회복(`recovery`) 신설 — 공용 (3,1), 선행 없음.** `Type=HealAura`, 6초 동안 매초 `20 + 스킬레벨×2` 회복(반경 2, 쿨 20초, MP 무소모). 공격력·마법력과 완전히 무관해 초반 생존에 바로 쓰인다. 와일드키퍼 `자연의 활력`(광역 힐 오라 + 가축 친밀도)의 자기 전용 약화판이라 직업 정체성을 침범하지 않는다.
  - ⚠️ 전용 아이콘 RUID 미지정(`자연의 활력` 과 동일한 선례). 노드는 이름 폴백으로 렌더된다. 공식 리소스 확보 시 `IconRUID` 만 채우면 된다.

| 스킬명 | 트리 좌표 | 선행 스킬 | 소모 자원 | 판정 및 특징 |
|---|---|---|---|---|
| **주먹도끼 던지기** (`hand_axe_throw`) | (2,2) | 루트 | 도끼 1개 | 단일 투사체 투척. 은퇴 모험가의 원시 손버릇. |
| **기간틱 락** (`gigantic_rock`) | (3,2) | 주먹도끼 Lv 3 | 도끼 1개 + **마나(MP)** | 마나를 주입해 거대한 거석으로 팽창. 관통 넉백 및 착탄 시 광역 지진파 피해. |
| **쇄석 흩뿌리기** (`shatter_scatter`) | (3,3) | 주먹도끼 Lv 3 | 도끼 1개 + **마나(MP)** | 마력으로 돌을 파쇄해 전방에 날카로운 부싯돌 조각 산탄 방사. 초근접 다단히트. |

### 8.2 데이터 계약

- **`ParentRequiredLevel` 컬럼**: 자식 해금(0→1)에 필요한 **부모 스킬 레벨**. 공란/누락 = 1 (pcall 가드).
  - 기간틱 락 / 쇄석 흩뿌리기 ← 주먹도끼 던지기 **Lv 3**.
  - 슬래시 블러스트 ← 파워 스트라이크 **Lv 3** / 강력 채집 ← 신속 채집 **Lv 3** / 매직 클로 ← 비전 마나 친화 **Lv 3**(알케미스트 탭). 회복은 선행 없음.
- 서버 게이트 ①(선행 스킬)을 "부모 Lv ≥ ParentRequiredLevel"로 검증.

### 8.3 단순성 가드라인 (⚖️ "너무 복잡한 구조는 싫어" — 트리 저작 규칙)

1. **단일 부모** — `ParentSkillId` 1개, 다중 선행 금지 (자식 기준 단일 부모이므로 1:N 분기 허용).
2. **트리 저작 좌표 규칙**: 부모는 같은 열 또는 인접 열의 바로 위 행(TreeRow−1)에 위치.
3. **노드 UI 표현**: 부모 노드에서 분기되어 아래 자식 노드로 연결되는 직관적 링크 시각화.

### 8.4 UI 설계

- **연결선**: 노드 슬롯(`Node_<r>_<c>`) 사이 커넥터 스프라이트(`Link_<r>_<c>`). 컨트롤러가 `ParentSkillId`에서 파생 표시.
- **상태 컬러**: 잠김=dim / **해금 가능=금색 하이라이트** / 보유=금색 실선 + Lv 뱃지.
- **SkillDetailPanel 선행 조건**: "선행: 주먹도끼 던지기 Lv 3 (현재 1)" — 충족=금색/미충족=적색.
- **직업 탭 확장 연동**: 탭 필터링(`JobId == 현재 탭`)으로 공용 탭과 직업 탭을 완전 분리 렌더.

구현 티켓: handoff §3 **T58** (스킬트리 시각화 및 직업/주먹도끼 분기 연계).
