...
| `PortalDestinationDataSet.csv` · `ResourceSpawner.mlua` · `Monster.mlua` · `UIWarpController.mlua` · `ui/PopupGroup.ui` · `map/template_snow.map` · `SlimeKing.model` · `StoneGolem.model` · `Deu.model` · `BossContentsDataSet.csv` · `BossDifficultyDataSet.csv` · `QuestDataSet.csv` · `QuestConditionDataSet.csv` · `StoryDialogDataSet.csv` · `scripts/build_all_hunting_maps.cjs` · `docs/design/story/hunting-grounds-plan.md` | **[사냥터/레이드/퀘스트] 상위 컨셉 사냥터 3구역 체인화 & 관문 보스 3종 레이드 편입 & 보스 해금 우회 경로 차단 & 파티 전원 해금 & 관문 퀘스트 217·227·236·246 신설** (2026-09-23 ⚖️ 제작자 확정) — **발견(정적 검증)**: 스톤골렘·데우·스노우맨이 어디에서도 스폰되지 않았고(`BossContentsDataSet` 슬라임킹 1행뿐, `SpawnHuntBoss` 호출처 0), `rocky01→desert01→snow01` 도보 전진 포탈 + `ServerRequestWarp` 의 현재 맵 자동 해금 때문에 **슬라임킹 이후 보스 해금이 걸어서 우회**됐다. ① **3구역 체인**: `rocky/desert/snow` 01~03 (각 컨셉 공용 템플릿). `PortalDestinationDataSet` 에 `ChainPrevId` 컬럼 신설 → `EnsureHuntingGroundMaps` 를 하드코딩 7맵에서 **데이터 주도 체인**으로 교체(흙 벌판 포함). 컨셉 간 도보 연결 제거 — 다음 컨셉 1구역은 보스 처치로만 열린다. 연결할 곳 없는 템플릿 포탈(체인 첫 구역 `PortalBack` — 구 `rocky01` 의 빈 목적지 포탈 결함)은 `DisableUnlinkedPortal` 로 끈다. ② **보스 레이드 편입**: `BossContentsDataSet` 에 `stone_golem`(Lv8, 선행 `rocky03`)·`deu`(Lv12, `desert03`)·`snowman`(Lv16, `snow03`) + 난이도 4단 12행(슬라임킹 배율·EntryLevel +0/+2/+6/+11 규약, 드롭은 전 난이도 공통 SourceId, 코인만 차등). 각 3구역 `PortalForward` 에 대기실 포탈이 `EnsureBossLobbyPortals` 로 자동 부착. 보스방은 공용 `template_boss`. ③ **`template_snow` `PortalForward` 추가** (MapBuilder, 사막 템플릿 전진 포탈 구성 복제, ExitCamp 광장 `(22,23)` — 사막과 레이어 점유 동일 확인). 생성 스크립트 snow 레이아웃도 동기화. ④ **파티 전원 해금**: `Monster:GrantBossRewards` 의 웨이포인트 해금 대상을 막타자 1인 → **보스가 죽은 맵의 전원**(`GetUsersByMapComponent`)으로. `[BOSS] ... unlocked waypoint ... for N player(s)` 로그. ⑤ **슬라임킹 확정 드롭 `iron_ore` → `copper ore`**(1막 자원, 보스마다 자기 막 광석). HARD/EXTREME 난이도 테이블의 철 보너스 행은 유지. 해금 토스트명도 새 구역명으로. ⑥ **워프 창 슬롯 10 → 13** (`Slot11~13` 은 `Slot10` 컴포넌트 그대로 복제, 의미 diff = 3엔티티 추가뿐 · `SlotCount=13`). ⑦ **관문 퀘스트 4종** 217→227→236→246 (`Kill` 1회, RequiredLevel = 레이드 입장 레벨) + 대사 22행. 221~226 등 챕터 퀘스트가 생기면 RequiredId 재배선 필요. 검증: CSV 컬럼 수 전 파일 일치(BOM·CRLF 보존) · `check_dataset_columns` 불일치 0 · `ui_lint` Error 0 (Warning 548→551 = 신규 슬롯 3건, 기존 Slot10 과 동일 유형) · `ModelBuilder` 재조회 값 대조 · `maker_refresh_workspace` ok · build 로그 15:37:18 (마지막 `.mlua` 수정 15:37:16 이후) **Error 0 / 신규 Warning 0** · 잔여 Warning 9건은 15:18:27 스냅샷의 **기존** `LWA-4012`(snowman·stone_golem·deu·catus 모델 값 타입 불일치 — 이번 변경 무관, 미조치) · 런타임 검증 보류(제작자 Play 수행). **미해결**: 퀘스트 `Kill` 진행은 여전히 막타자에게만 발행(`Monster.mlua` ActionSignals) · 보스 3종 모두 `LEAP` 단일 패턴 · 상시 사냥터 맵 7 → 13개로 서버 상주 맵 증가(성능 Play 확인 필요). |
| `docs/design/story/hunting-grounds-plan.md` · `docs/design/story/hunting-grounds-ecosystem-plan.md` · `docs/design/story/README.md` · `docs/design/story/story-bible.md` · `docs/design/story/quest-design.md` · `docs/design/story/map-concepts.md` · `docs/design/magic-skills-and-rare-equipment.md` · `game_design.md` | **[기획/문서정합] 사냥터 맵 구조(초보 벌판 3단계 hunt01~03 보존) & 관문 보스 순차 해금 체계 및 히든 사냥터 절대 격리 원칙 전면 동기화** (2026-09-22 ⚖️ 확정) — 과거 구형 초안에 남아 있던 가상 보스 명칭(`dew_glutton`, `echo_warden`, `well_keeper`) 및 구형 맵 매핑(`hunt01`~`hunt04`를 챕터 2~5에 1:1 대응시키거나 슬라임킹을 챕터 5에 두었던 오류)을 전면 제거하고 실제 구현 아키텍처로 일치화 완료. ① **1막 초보 흙 벌판 3단계 (`hunt01`~`hunt03`) ➔ 1막 관문 보스 슬라임킹(`slime_king` / `template_boss`)** 정립. ② **관문 보스 격파 시 상위 컨셉 순차 해금**: 슬라임킹 격파 ➔ 2막 바위 고원(`rocky01`, 스톤골렘) 해금 ➔ 3막 모래 언덕(`desert01`, 데우) 해금 ➔ 4막 만년 설원(`snow01`, 스노우맨) 해금. ③ **프로젝트 맵 템플릿(`map/template_*.map`, `map/field_earth.map`) vs 런타임 맵 인스턴스 동적 생성 체계** 문서화. ④ **히든 사냥터 원칙**: 중앙 단일 포탈(`WarpPopup` / `PortalDestinationDataSet`) 목록에 절대 노출·연결되지 않고 전용 가구/비밀 통로로만 진입하도록 못박음. |
| `JrYeti.model`(신규) · `Yeti.model`(신규) · `Pepe.model`(신규) · `WhiteFang.model`(신규) · `Snowman.model`(신규) · `template_rocky.map`(신규) · `template_desert.map`(신규) · `template_snow.map`(신규) · `template_field.map` · `field_earth.map` · `PortalDestinationDataSet.csv` · `item_dataset.csv` · `MonsterSpawnDataSet.csv` · `ItemDropDataSet.csv` · `MonsterCoinDropDataSet.csv` · `ResourceSpawner.mlua` · `SlimeKing.model` · `StoneGolem.model` · `Deu.model` · `scripts/create_f4_monsters.cjs`(신규) · `scripts/apply_f4_ecosystem_data.cjs`(신규) · `scripts/build_all_hunting_maps.cjs`(신규) | **[사냥터/생태계/맵] 초보 벌판 3단계(hunt01~03)→슬라임킹 보스 체인 원상 복원 및 컨셉별 전용 맵 템플릿(template_rocky/desert/snow) 구축 & 상위 웨이포인트(rocky01/desert01/snow01) 보스 해금 연동** (2026-09-22) — ① **초보 사냥터(벌판) 3단계 체인 보존**: `hunt01`~`hunt03`은 초보 사냥터인 벌판(`earth_field` 바이옴, `field_earth`/`template_field` 템플릿)으로 유지. `hunt01`➔`hunt02`➔`hunt03` 순차 진행 후 3구역 끝에서 슬라임킹 레이드 로비로 진입하는 정통 루트 복원. ② **컨셉별 전용 맵 템플릿 프로젝트 에셋 구축**: 업로드된 `wall.tileset` 타일셋(Rock 13종, Sand 13종, Snow 13종)을 바탕으로 컨셉별 맵 템플릿 파일 구축(`map/template_rocky.map`, `map/template_desert.map`, `map/template_snow.map`). ③ **인게임 맵 인스턴스화 & 슬라임킹 이후 상위 컨셉 해금**: 슬라임킹 처치 시 `rocky01`(바위 고원) 해금(`SlimeKing.model`의 `UnlockWaypointId = "rocky01"`), 스톤골렘 처치 시 `desert01`(모래 언덕) 해금(`StoneGolem.model`의 `UnlockWaypointId = "desert01"`), 데우 처치 시 `snow01`(만년 설원) 해금(`Deu.model`의 `UnlockWaypointId = "snow01"`). `ResourceSpawner.mlua`에서 각 템플릿으로부터 인게임 맵 인스턴스 생성 및 포탈 연결. ④ **F4 만년설원 몬스터 5종 모델 및 전리품 5종·스폰·드롭 데이터셋 완비**. ⑤ **히든 사냥터 격리 정책**: 웨이포인트 UI에는 정규 사냥터만 노출되며, 향후 추가될 히든 사냥터는 단일 포탈에 노출되지 않고 독립 전용 진입점으로만 열리도록 정책 분리. 검증: `check_dataset_columns` ✅ 불일치 0 · 5개 템플릿 맵 직렬화 무결성 확인 · `maker_refresh_workspace` status ok · build 로그 갱신 확인(Error 0 / Warning 0 / Info 723, 18:34:37) · 런타임 검증 보류(제작자 Play 수행) |
| `Moredji.model`(신규) · `Catus.model`(신규) · `Scorpion.model`(신규) · `Bellamoa.model`(신규) · `Deu.model`(신규) · `scripts/create_f3_monsters.cjs`(신규) · `scripts/apply_f3_ecosystem_data.cjs`(신규) · `MonsterSpawnDataSet.csv` · `item_dataset.csv` · `ItemDropDataSet.csv` · `MonsterCoinDropDataSet.csv` | **[사냥터/생태계] F3 모래 사막(desert) 몬스터 5종 모델 및 전리품 7종·스폰·드롭 데이터셋 구축 — 데우 B3 보스는 슬라임킹 LEAP(도약 강타) + AttackTelegraph 장판 구성 차용** (2026-09-22) — ① **모델 5종**(`ModelBuilder`, 기존 모델 복제 기반): `Moredji`(slug `moredji`, Slime 기반 `CONTACT` · HP120/Def16 · InputSpeed 2.4 · SpawnGraceDuration 0.6 으로 "잠복 기습" 근사) / `Catus`(`catus`, **HornMushroom 기반** `RANGED` · HP140/Def20 · InputSpeed 1.2 = 사실상 제자리 · StopDistance 6.0 을 사거리로 사용 · `ProjectileModelId="Projectile_Spore"` 재사용 / ProjectileDamage 16 · Speed 5.0 · LifeTime 2.2 · ContactDamage 0 + TouchDamage 4) / `Scorpion`(`scorpion`, Slime 기반 `MELEE_WEAPON` · HP150/Def18 · ContactDamage 18 = F3 최고 단타 · AttackCooldown 1.4 = F3 최단 쿨) / `Bellamoa`(`bellamoa`, Slime 기반 `CONTACT` · HP130/Def14 · InputSpeed 3.1 = 슬라임 1.3배 · LeashRange 15) / `Deu`(`deu`, SlimeKing 기반 `LEAP` 보스 · **HP2600/Def26 · IsBoss** · SlamDamage 36 / SlamRadius 3.0 · `TelegraphModelId="AttackTelegraph"` · LeapReactionWindow 0.75 · `LeapAirFrameIndex 5`(skill1 12프레임 대응) · ImpactShake 0.7/0.4 · **BossDropItem `soft_sand` 6~12**). 상속된 슬라임킹 값 중 **미니언 소환을 `slime`→`moredji`(30초·3기)로 재테마**하고 **`UnlockWaypointId` 를 `hunt02`→빈 문자열로 강제 클리어**(아래 ⑥). ② **전리품 7종** `soft_sand`/`mole_claw`/`cactus_thorn`/`cactus_flower`/`scorpion_stinger`/`chitin_shell`/`snake_scale` 를 `item_dataset.csv` 에 추가(기존 `pig_ribbon` 행을 템플릿 복제해 37컬럼 정합 보장, 설명문 쉼표 0 — 함정 규칙 51). ③ `ItemDropDataSet` 11행·`MonsterCoinDropDataSet` 5행 추가. 데우의 `soft_sand` 대량 드롭은 `Monster.BossDropItem` 이 담당하므로 드롭 테이블에는 중복 배정하지 않고 테마 전리품(`chitin_shell` 3~6 / `cactus_flower` 2~4)·`recipe_scroll_iron` 만 배정(스톤골렘·슬라임킹 선례). 퀘스트 233 `Gather,cactus_thorn,6` 공급원은 `catus` 단독(0.5 × 1~2개). ④ **`MonsterSpawnDataSet` desert 라인업 교체** — 기획서 §2.3 대로 `moredji(70)`/`catus(60)`/`scorpion(50)`/`bellamoa(40)` 4종으로 두고 **기존 `desert,slime`·`desert,boar`·`desert,horn_mushroom` 배율 재탕 3행 제거**(F1·F2·F4 행은 불변). ⑤ **원거리·도약 동시 구현 불가 규명**: `MonsterAI:FireProjectile()` 은 `ATTACK` 상태의 스트라이크 분기(`MonsterAI.mlua:318`)에서만 호출되고 `AttackType="LEAP"` 은 `EnterState("LEAP")` 로 분기되므로, **LEAP 보스에 `ProjectileModelId` 를 채우면 영원히 발사되지 않는 죽은 설정**이 된다 → 데우는 `ProjectileModelId=""` 로 명시 클리어하고 Damage 22 / Speed 6.5 / LifeTime 2.0 / HitRadius 0.5 만 선튜닝해 둠(전용 보스 AI 도입 시 한 줄 교체로 활성화). ⑥ **미착수**: (a) 기획서의 데우 3패턴(모래 소용돌이 흡인 / 가시 폭풍 5연속 예고원 / 메마름의 저주) 중 예고원→솟구침만 `LEAP` 로 근사, 나머지 2패턴은 `MonsterAI.AttackType` 단일 값 구조상 구현 불가(F2 스톤골렘과 동일 제약). (b) **`UnlockWaypointId` 공란** — B3 다음 목적지(B5 첫 정원 / F4 설원)가 `PortalDestinationDataSet` 에 아직 없어 잘못된 해금을 막기 위해 비워 둠. 행이 생기면 `Deu.model` 의 2줄만 채우면 됨. (c) 스콜피온 "중독 타격" — 몬스터→플레이어 상태이상 파이프라인 미구현(색인 §5 미해결)이라 순수 물리 피해로 근사. (d) 카투스 가시 침 전용 투사체 스프라이트 미제작(포자 재질 그대로). (e) 맵 배치/스포너 연결 미착수. 검증: 두 스크립트 **재실행 멱등 확인**(전 행 `skip (exists)`, desert 블록 idempotent 재생성) · 전 CSV **BOM·CRLF 보존 실측**(4파일 모두 BOM=true, CRLF=LF 동수) · 컬럼 수 일치(따옴표 인식 파서, 불일치 0: item 37 / spawn 5 / drop 8 / coin 6) · 드롭 `ItemId` 11건 전부 `item_dataset` 실재 확인 · `check_dataset_columns` ✅ 불일치 0 · `ModelBuilder` 스냅샷으로 5모델 핵심 값 실측 대조 · refresh `status ok`(2회) · **build 로그 갱신 미확인** — 2회 refresh 후에도 마지막 스냅샷 `15:25:49`(현재 시각 18:16)가 그대로 재반환됨(함정 규칙 22), 따라서 그 스냅샷의 Error 0 / Warning 0 / Info 723 은 이번 변경의 근거로 쓰지 않음. 신규 `.mlua` 0건이라 `.codeblock` 생성 확인 대상 없음 · 런타임 검증 보류(제작자 Play 수행) |
| `Stump.model`(신규) · `IronHog.model`(신규) · `StoneGolem.model`(신규) · `scripts/create_f2_monsters.cjs`(신규) · `scripts/apply_f2_ecosystem_data.cjs`(신규) · `MonsterSpawnDataSet.csv` · `item_dataset.csv` · `ItemDropDataSet.csv` · `MonsterCoinDropDataSet.csv` | **[사냥터/생태계] F2 바위 지대(rocky) 몬스터 3종 모델 및 전리품 4종·스폰·드롭 데이터셋 구축 — 스톤골렘 B2 보스는 슬라임킹 LEAP(도약 강타) + AttackTelegraph 장판 구성 차용** (2026-09-22) — ① **모델 3종**(`ModelBuilder`, 기존 모델 복제 기반): `Stump`(slug `stump`, Slime 기반 `CONTACT` · HP70/Def14 · InputSpeed 1.45 = 슬라임 0.6배) / `IronHog`(`iron_hog`, Boar 기반 `CHARGE` · HP85/Def10 · ChargeSpeedMultiplier 3.6 · AttackRUID=`skill1` 돌진 모션) / `StoneGolem`(`stone_golem`, SlimeKing 기반 `LEAP` 보스 · HP1400/Def22 · IsBoss · SlamDamage 30 / SlamRadius 2.6 · `TelegraphModelId="AttackTelegraph"` · LeapReactionWindow 0.7 · `LeapAirFrameIndex 4`(skill1 10프레임 대응) · BossDropItem `iron_ore` 5~10 · `UnlockWaypointId="hunt03"`). 상속된 슬라임킹 값 중 **미니언 소환을 `slime`→`stump`(35초·3기)로 재테마**. ② **전리품 4종** `stump_wood`/`stump_leaf`/`iron_fragment`/`horn_fragment` 를 `item_dataset.csv` 에 추가(기존 `pig_ribbon` 행을 템플릿 복제해 37컬럼 정합 보장, 설명문 쉼표 0 — 함정 규칙 51). ③ `ItemDropDataSet` 7행·`MonsterCoinDropDataSet` 3행 추가. 스톤골렘의 `iron_ore` 대량 드롭은 `Monster.BossDropItem` 이 담당하므로 드롭 테이블에는 중복 배정하지 않음(슬라임킹 선례). ④ **`MonsterSpawnDataSet` rocky 라인업 교체** — 기획서 §2.2 대로 `stump(70)`/`iron_hog(60)`/`horn_mushroom(60,1.6,1.5)` 3종으로 두고 **기존 `rocky,slime`·`rocky,boar` 행 제거**(재탕 구조 탈피). ⑤ **색인 오류 2건 정정**: (a) RUID 색인 §0 규칙 2의 "Scale.x 부호 반전"은 실제 구현과 불일치 — `MonsterAI:Face()`(MonsterAI.mlua:1555)는 `SpriteRendererComponent.FlipX` 를 쓰므로 모델에서 Scale 을 손댈 필요 없음. (b) 색인이 `iron_hog` 에 제안한 `TelegraphModelId=AttackTelegraph` 는 **무효** — 지면 예고 장판은 `EnterState("LEAP")` 에서만 스폰되고 `CHARGE` 의 예고는 `TelegraphOn` 주황 틴트뿐이라, IronHog 에는 설정하지 않음. ⑥ **미착수**: 기획서의 스톤골렘 3패턴 중 ②8방향 파편·③엄폐 요구 광역기는 `MonsterAI.AttackType` 단일 값 구조상 구현 불가(전용 보스 AI 신설 필요) — 1차는 `LEAP` + 미니언 소환. 맵 배치/스포너 연결도 미착수. 검증: 두 스크립트 **재실행 멱등 확인** · 전 CSV 컬럼 수 일치(따옴표 인식 파서, 불일치 0) · `check_dataset_columns` ✅ 불일치 0 · `ModelBuilder` 스냅샷으로 26개 핵심 값 실측 대조 · refresh `status ok` · **build 로그 갱신 미확인**(`.model`/`.csv` 단독 변경이라 새 빌드 로그 없음 — 마지막 스냅샷 15:25:49, Error 0 / Warning 0 / Info 723) · 런타임 검증 보류(제작자 수행) |
| `RibbonPig.model` · `OrangeMushroom.model` · `MonsterSpawnDataSet.csv` · `item_dataset.csv` · `ItemDropDataSet.csv` · `MonsterCoinDropDataSet.csv` · `tileimg/*.png` | **[사냥터/생태계] F1 흙 벌판 신규 몬스터(리본돼지·주황버섯) 모델 및 전리품·스폰·드롭 데이터셋 구축 & F2/F3/F4 3대 바이옴 타일셋 에셋(바위/사막/설원 베이스 및 12타일 스트립) 제작 완료** (2026-09-22) — ① Claude CLI 연동을 통해 공식 메이플스토리 리소스 실조회 RUID 색인 및 선정(`docs/design/story/hunting-grounds-ruid-index.md` 확정). ② F1 신규 몬스터 모델 2종(`RibbonPig.model`, `OrangeMushroom.model`) ModelBuilder 기반 생성. ③ `item_dataset.csv` 신규 전리품 3종(`pig_ribbon`, `orange_mushroom_cap`, `boar_leather`) 추가, `MonsterSpawnDataSet.csv`에 F1 스폰 가중치 반영, `MonsterCoinDropDataSet.csv` 코인 드롭 및 `ItemDropDataSet.csv` 전리품 드롭율 배정 완료. ④ F2(바위), F3(사막), F4(설원) 지형 구축용 베이스 타일(64×64) 및 12타일 프린지 스트립(768×64) 6종 생성 완료(`rock.png`, `rock_fringe_12tiles_strip.png`, `sand.png`, `sand_fringe_12tiles_strip.png`, `snow.png`, `snow_fringe_12tiles_strip.png`). 제작자 MSW 업로드 및 RUID 발급 대기. 검증: `check_dataset_columns` ✅ 불일치 0 · refresh ok · build Error 0 / Warning 0 · 런타임 검증 보류(제작자 수행) |
| `TileDurabilityManager.mlua` · `docs/pitfalls.md` | **[영지/버그픽스] 점유 겹침으로 엉뚱한 가구가 철거되고 남은 가구가 영구 타격 불가가 되던 결함 해소** (2026-09-22) — ① **증상**: 나란히 붙은 나무 상자 2개 중 아랫것은 타격되는데 윗것은 아무리 때려도 무반응. ② **원인(런타임 로그 실측)**: `gridToEntity` 는 셀당 슬롯이 하나인데, 두 상자의 2×2 점유가 한 칸에서 겹쳐 나중에 등록된 아래 상자가 **위 상자의 피벗 칸 `3_-1` 을 가져갔다**. `HitResource` 가 `gridToEntity[pivotKey]` 를 재조회하면서 엉뚱한 가구를 잡아 `Furniture demolished: ... at 3_-1` 인데 `Removed placed furniture at (3, -2)` 가 찍혔고, 철거 시 `RemoveEntity` 가 겹친 칸까지 비워 위 상자는 피벗 칸을 잃고 영구 타격 불가가 됐다. ③ **겹침의 출처**: 커밋 `3a84a64` 에서 가구 점유를 1×1 → 2×2 로 바꾸면서, 그 이전에 한 칸 간격으로 설치돼 저장된 배치들이 로드 시 소급 적용돼 겹쳤다. 신규 설치는 서버 2×2 검사가 막고 있어 레거시 데이터에서만 발생. ④ **조치(A안)**: `HitResource` 가 `gridToEntity[pivotKey]`의 소유 피벗이 `pivotKey` 와 일치하는지 검사하고, 불일치/부재 시 그 피벗을 가리키는 다른 점유 칸에서 복구하도록 가드 신설. 함정 규칙 46(인자 수 변경 금지) 때문에 **시그니처는 유지**. 함정 규칙 60 등록. ⑤ **미착수(B안)**: 이미 겹쳐 저장된 레거시 배치 자체의 정리는 보류 — 저장된 영지 데이터를 건드리는 작업이라 제작자 판단 대기. 렌더 크기 ↔ 점유 칸 구조 재설계도 함께 검토 예정. 검증: build Error 0 / Warning 0 (로그 04:25:33 = refresh 시각 일치) · 런타임 검증 보류(제작자 수행) |
| `Furniture_WoodenChest`.model`` · `Furniture_Furnace`.model`` · `PlayerController.mlua` · `item_dataset.csv` | **[영지/크기] 상자 충돌박스 과대 & 화로 스프라이트 과대 정정 & 배치 미리보기 배율 데이터화** (2026-09-22) — ① **상자**: 스프라이트 64×48px @ `Scale` 2 = 1.28×0.96 units 인데 `TriggerComponent.BoxSize` 가 1.7×1.3(가로 33%·세로 35% 초과)이었다 → **1.28×0.96** 로 정정. `ColliderOffset` 은 피벗 정보가 없어 미변경. ② **화로**: 스프라이트 268×168px @ `Scale` 2 = **5.36×3.36 units**(2×2 점유의 2.7배)였다 → `Scale` **0.75**(2.01×1.26)로 축소. 원본 해상도가 상자의 4배인데 `Scale` 을 상자와 같이 2로 둔 게 원인. ③ **미리보기**: `PlayerController:UpdatePlacementPreview` 의 배율이 `is2x2 면 무조건 2.0` 하드코딩이라 모델 `Scale` 과 따로 놀았다 → `item_dataset.PreviewScale` 컬럼(37번째) 신설, 화로 0.75 지정, 공란은 기존 동작 유지. CSV 는 따옴표 인식 파서로 전 행 36필드 일치 확인 후 추가(함정 규칙 51). ④ 잔여: 화로 Idle(268×168)과 Active(280×280) 스프라이트 종횡비 불일치는 미조치. 검증: 스프라이트 픽셀 크기 리소스 API 실측 · build Error 0 / Warning 0 (로그 04:17:21 일치) · `check_dataset_columns` 불일치 0 · 런타임 검증 보류(제작자 수행) |
| `BossContentsDataSet.csv` | **[레이드/리소스] 보스 컨텐츠 창 슬라임킹 섬네일이 이펙트 이미지로 나오던 결함 수정** (2026-09-22) — `ArtRUID` 가 `13ce4f3c…` 였는데, 리소스 인덱스에는 `type=sprite` · `category=mob` · 이름 "킹슬라임" 으로 **정상 등록돼 있지만 실제 이미지는 희미한 흰 연기 이펙트**였다(썸네일 직접 다운로드해 육안 확인). 킹슬라임 팩 `mob/9300003.img` 는 본체 7종과 이펙트·사운드가 모두 같은 이름으로 색인돼 있어, 팩을 거치지 않고 개별 검색으로 집으면 이런 게 걸린다. → `stand` 애니메이션 첫 프레임 **`365720771a6a446d8daa213037b0ab45`**(`sprite`, 208×144)로 교체, 본체 확인 완료. 사용처는 `UIBossContentsController:277` 한 곳. 검증: 교체 전후 썸네일 육안 확인 · `check_dataset_columns` 불일치 0 · refresh ok · 런타임 검증 보류(제작자 수행) |
| `docs/design/story/hunting-grounds-ecosystem-plan.md`(신규) · `docs/design/story/README.md` | **[사냥터/몬스터/퀘스트] 사냥터별 공식 메이플 몬스터 생태계(F1 리본돼지·주황버섯, F2 스텀프·아이언호그·스톤골렘, F3 모래두지·카투스·스콜피온·데우, F4 예티·페페·화이트팽·스노우맨) & 챕터 2~4 퀘스트 라인 및 연구소 연계 상세 기획 확정** (2026-09-22 ⚖️ 확정) — 기존 3종(슬라임/멧돼지/뿔버섯) 재탕 구조를 탈피하고 바이옴 테마에 맞는 메이플 공식 몬스터 및 보스 패턴, 신규 전리품 ↔ 마을 연구소 ↔ 대장간/생활 테크 연계, 퀘스트 엔진 하드 제약(11종 ActionEnum)을 준수한 챕터별 퀘스트 체인 상세 기획서 저장 완료. |
| `ui/PopupGroup.ui` | **[UI/스크롤] 휠이 '항목 위'에서만 먹던 원인 규명(컨테이너 `RaycastTarget`) & 세로 스크롤바 핸들 역방향 수정** (2026-09-22) — ① **핸들 역방향**: 퀘스트·제작·도감 3곳의 `VerticalScrollBarDirection` 이 `3`(TopToBottom)이었다. 세로 목록 정상값은 엔진/빌더 기본 `2`(BottomToTop). 3곳 정정, 확인 완료. ② **휠 씹힘(본 원인)**: 휠은 커서 아래 레이캐스트 대상이 있어야 전달된다. 제작대·퀘스트는 스크롤 컨테이너 스프라이트의 `RaycastTarget` 이 `false` 라 **자식 행(Button) 위에서만** 동작했고, 행 간격(`Spacing` 10)·행 폭과 컨테이너 폭 차이(400 vs 420)·목록 끝 빈 공간이 사각지대였다. 도감만 `true` 여서 정상이었던 것. 두 곳을 `true` 로 변경. ③ **오진 기록**: 제작자가 처음 제기한 '항목 위 vs 목록 위' 가설을 내가 계측으로 기각했는데 **그게 틀렸다** — 좌표만 봤고 엔진의 스크롤 래치(항목 위에서 시작하면 잠시 벗어나도 계속 먹음)를 고려하지 않아 '같은 좌표, 다른 결과'를 위치 무관의 근거로 잘못 읽었다. 제작자 재계측이 맞았다. 함정 규칙 59에 오진 패턴까지 기록. ④ **철회분**: 휠 끊김 완화용 `UISmoothScroll` 은 **엔진이 이미 다중 프레임 감속 보간 중**이라 충돌해 더 버벅였고 전량 제거(`.mlua`/`.codeblock` 삭제). 계측 결과 엔진 스텝은 고정 ~31px 양자화(제작대 `1/32`, 도감 `1/137`)라 짧은 목록일수록 거칠어 보이는 것이며, 스크립트로 개선할 영역이 아니다. ⑤ 계측용 `UIScrollProbe` 도 역할 종료 후 제거. 잔여 참조 0 확인. 검증: HEAD 대비 `ui/PopupGroup.ui` 실변경 **5건만** 확인(방향값 3 + `RaycastTarget` 2, 수치 비교) · refresh ok · 잔여 참조 0 · `.u`+`i` 단독 변경은 빌드 로그를 새로 남기지 않아 **build 로그 갱신 미확인**(마지막 빌드 Error 0 / Warning 0) · ②의 런타임 확인 보류(제작자 수행) |
| `scripts/check_dataset_columns.cjs`(신규) · `docs/pitfalls.md` · `.mlua` 12종 주석 | **[데이터셋/정합] 잔여 `pcall` 열 프로빙 전수 감사 & 열 이름 드리프트 정적 검사기 신설 & 잘못된 함정 지침 교정** (2026-09-22) — ① 전수 감사: `pcall` 열 프로빙 114곳(총 열 참조 331곳)을 CSV 헤더와 대조한 결과 **실재하지 않는 열을 찌르는 곳은 0건**. 화로가 유일한 실사례였고 이미 해소됨. 매 프레임(`OnUpdate`) 경로의 프로빙도 화로 외에는 없음. ② `scripts/check_dataset_columns.cjs` 신설 — `.mlua`의 `GetItem("X")`/`GetCell(_, "X")` 를 `_DataService:GetTable` → `FindRow`/`GetRow` 변수 역추적으로 소속 데이터셋까지 확정해 대조하고, `.model`의 `*TableName`↔`*Column` 짝(ModelBuilder 경유)도 검증한다. 불일치 시 종료 코드 1. **자체 검증**: 화로 모델 값을 `Duration` 으로 되돌려 넣었더니 해당 줄을 정확히 지목하고 종료 코드 1 반환 → 원복 확인. (코드 리터럴만 보는 1차 버전은 이 사고를 못 잡아서 모델 설정값 경로를 추가한 것.) ③ 함정 규칙 7 개정 — "`pcall` 가드 필수"라는 **틀린 지침**(이번 사고의 직접 원인)을 폐기하고 ①동적 열 이름은 `Columns:Contains` 선확인 ②리터럴 열 이름은 검사기로 정적 보장 2갈래로 정리. ④ `pcall`이 LEA-3011을 막는다고 잘못 서술한 주석 12곳(`ResourceSpawner`/`Monster`/`BossRaidLogic`/`QuestData`/`UISkillTree` 등) 교정 및 삭제된 `Furnace readDur` 선례 참조 제거. ⑤ **잔여 `pcall` 가드 114곳은 의도적으로 일괄 제거하지 않음** — 전부 실재 열이라 런타임 이득이 없고, 저장(`PersistenceManager`)·전투 경로까지 30개 파일을 건드리는 고위험 변경이라 판단(함정 규칙 7에 사유 기록). 검증: `check_dataset_columns` ✅ 불일치 0 · refresh ok · build Error 0 / Warning 0 (로그 01:54 = refresh 시각 일치) · 런타임 검증 보류(제작자 수행) |
| `Furnace.mlua` · `UIFurnaceController.mlua` · `Furniture_Furnace.model` | **[제작/버그픽스] 화로 제련 시 LEA-3011(`Duration`·`CookDuration` 열 없음) 로그 폭주 해소 — 모델 오타 값 정정 & 존재 열 선판정 방식으로 교체** (2026-09-21) — ① 원인: `Furniture_Furnace.model`의 `script.Furnace.DurationColumn` 값이 `"Duration"`(오타)이었고, `SmeltingRecipeDataSet.csv`의 실제 열 이름은 `SmeltDuration`. 없는 열이라 1순위 조회 실패 → 2순위 `CookDuration`(제련 테이블엔 없음)도 실패 → 3순위 `SmeltDuration`에서야 값을 얻어 기능 자체는 동작했으나 매 프레임 LEA-3011 2건이 찍혔다. ② `pcall`로 감싸도 LEA-3011은 엔진이 로그에 남기므로 억제되지 않는다(`UISkillTreeController:HasCategoryColumn` 선례와 동일). → `UserDataSet.Columns:Contains()`로 실재 열을 먼저 고르는 `Furnace:ResolveDurationColumn()` 신설(결과 `_T` 캐시, 후보 우선순위 `DurationColumn` → `SmeltDuration` → `CookDuration`, 전부 없으면 경고 1회). ③ 서버 `Furnace:OnUpdate`와 클라 `UIFurnaceController:GetSmeltDuration`이 같은 판정을 공유하도록 정리. ④ 모델 값 `Duration` → `SmeltDuration`(ModelBuilder). 냄비(`Furniture_CookingPot`)는 `CookDuration`으로 이미 정상. 검증: refresh ok · build Error 0 / Warning 0 (로그 23:18 = refresh 시각 일치) · `.codeblock` 재생성 확인 · 런타임 검증 보류(제작자 수행) |
| `VillagerDialog.mlua` · `UserQuestData.mlua` · `QuestData.mlua` · `UIQuestController.mlua` · `UIQuestNavigationController.mlua` | **[퀘스트/네비] 보고 가능 상태 표시 & 자격 미달 진행 퀘스트 마크 버그 수정** (2026-09-19) — ① 보고처 판정을 `UserQuestData:IsReadyToTurnIn`(조건·자격·넘길 물건, 서버 `Complete`와 같은 게이트) 한 곳으로 모음. ② NPC 마크 `GetQuestPingKind`에 `"done"` 추가(우선순위 done > ! > ?) → 보고 가능하면 **초록 "!"**. F 대화도 보고를 새 제안보다 먼저 처리. ③ HUD 트래커: 보고 가능 퀘스트를 자동 추적 1순위로, 안내 줄 `[보고 가능] <NPC>에게 돌아가 보고하세요.`(초록) / 자격 미달이면 `[보고 불가] Lv.N 이상 필요`(주황). ④ 네비: 보고 가능 주민 우선 안내, 뱃지·나침반 색도 초록. ⑤ **버그 원인**(Maker 로그 실측): Lv.3 캐릭터에 구 게이트(Lv.2)에서 수락된 직업 준비 퀘스트 311·321이 진행 중으로 남아 대장장이·연구원에 "?"가 떴고, 대화하면 완료 페이지를 보여주며 `RequestCompleteQuests`를 보냈지만 서버 `IsPlayerEligible`(Lv.4)이 조용히 거절. → 자격 미달 진행 퀘스트는 마크·네비에서 숨기고, 대화는 사유(`QuestData:GetEligibilityBlockText`)를 안내. 검증: LSP error 0 · refresh ok · build Error 0 / Warning 0 (로그 23:39 = refresh 시각 일치) · 런타임 검증 보류(제작자 수행) |
