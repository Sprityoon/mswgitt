...
console.log("=== SlotPanel ===");
for (const row of b.listEntities()) {
  if (!/SlotPanel/.test(row.path || "")) continue;
  ...
  console.log(
    `${e.jsonString.name} do=${e.jsonString.displayOrder} enable=${e.jsonString.enable}` +
      ...
      ` pivot=${JSON.stringify(ut?.Pivot)}` +
      (spr ? ` spr=${hex(spr.Color)} type=${spr.Type} ruid=${JSON.stringify(spr.ImageRUID)} preserve=${spr.PreserveSprite}` : "")
  );
}
