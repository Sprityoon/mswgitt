/**
 * scripts/apply_water_border_to_field.cjs
 * 
 * 사냥터(template_field.map)의 기존 내부 사냥터 지형(|x| <= 27, |y| <= 27)을 100% 원형 보존하면서,
 * 기존 외곽 돌벽(|x| >= 28 or |y| >= 28) 및 그 바깥 영역(-55 ~ 55)에 '물(Water)'과 '수변 림(WetRim)', '해변 잔디 에지'를 완벽한 정방향으로 안착시킵니다.
 */
...
// MapComponent 설정
const mapEnt = ents.find(e => e.jsonString && e.jsonString.name === "template_field");
if (mapEnt) {
...
// 7. 기존 L2 잔디 타일 수집 (|x| < 27 and |y| < 27 내부 잔디는 100% 무결점 보존)
const l2Map = new Map();
(L2.tc.tileMap || []).forEach(t => {
  if (Math.abs(t.position.x) < 27 && Math.abs(t.position.y) < 27) {
    l2Map.set(`${t.position.x},${t.position.y}`, t.tileIndex);
  }
});
console.log(` - Preserved internal L2 Grass tiles: ${l2Map.size}`);

// 8. 해안선 경계셀(|x| == 27 or |y| == 27)의 잔디 에지 타일 정방향 계산 (gm |= bits)
for (let y = -27; y <= 27; y++) {
  for (let x = -27; x <= 27; x++) {
    const isBorder = Math.abs(x) === 27 || Math.abs(y) === 27;
    if (isBorder) {
...
// 10. 경계 콜라이더 (X = ±28.5, Y = ±28.5)
const bounds = [
  { name: "Boundary_Left",   x: -28.5, y: 0.0,   w: 2.0,  h: 58.0 },
  { name: "Boundary_Right",  x: 28.5,  y: 0.0,   w: 2.0,  h: 58.0 },
  { name: "Boundary_Top",    x: 0.0,   y: 28.5,  w: 58.0, h: 2.0 },
  { name: "Boundary_Bottom", x: 0.0,   y: -28.5, w: 58.0, h: 2.0 }
];
...
// 샘플 타일 방향 검증
const sampleChecks = [
  { desc: "Top Water (0, 28)", x: 0, y: 28, list: l0Tiles },
  { desc: "Bottom Water (0, -28)", x: 0, y: -28, list: l0Tiles },
  { desc: "Left Water (-28, 0)", x: -28, y: 0, list: l0Tiles },
  { desc: "Right Water (28, 0)", x: 28, y: 0, list: l0Tiles },
  { desc: "Top WetRim (0, 28)", x: 0, y: 28, list: l6Tiles },
  { desc: "Bottom WetRim (0, -28)", x: 0, y: -28, list: l6Tiles },
  { desc: "Top Grass Border (0, 27)", x: 0, y: 27, list: finalL2Tiles },
  { desc: "Bottom Grass Border (0, -27)", x: 0, y: -27, list: finalL2Tiles },
  { desc: "Left Grass Border (-27, 0)", x: -27, y: 0, list: finalL2Tiles },
  { desc: "Right Grass Border (27, 0)", x: 27, y: 0, list: finalL2Tiles },
  { desc: "Top-Left Grass Corner (-27, 27)", x: -27, y: 27, list: finalL2Tiles },
  { desc: "Top-Right Grass Corner (27, 27)", x: 27, y: 27, list: finalL2Tiles },
  { desc: "Bottom-Left Grass Corner (-27, -27)", x: -27, y: -27, list: finalL2Tiles },
  { desc: "Bottom-Right Grass Corner (27, -27)", x: 27, y: -27, list: finalL2Tiles }
];
...
console.log("Successfully applied perfect island water border to template_field.map!");
